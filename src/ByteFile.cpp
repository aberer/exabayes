...
void ByteFile::parse(ParallelSetup& pl, nat readerStride )
{
  auto numTax = int(0); 
  auto numPart = int(0); 
...
#else 
  parseAlnsDirect_newLayout(pl, pAss, readerStride); 
#endif 
...
void ByteFile::parseAlnsDirect_newLayout(ParallelSetup& pl, PartitionAssignment& pAss, nat readerStride )
{
  seek(Position::ALIGNMENT); 
  auto alnPos = _in.tellg();

  tout << SHOW(readerStride) << std::endl; 
  auto pureIo = 0.f; 
  auto pureReorder = 0.f;  
  auto pureParsimony = 0.f; 
  auto pureCommTime = 0.f; 

  // auto readerColor = pl.getChainComm().getRank() / readerStride; 
  // auto readerRank = pl.getChainComm().getRank() % readerStride; 
  // auto readerComm = pl.getChainComm().split({ int(readerColor) } , {   int(readerRank) } ); 

  // auto c = pl.getMyCoordinates(); 
  // std::cout  << "[" << c[0] << "," << c[1] << "," << c[2] << "] has color / rank " << readerColor << "\t" << readerRank << std::endl; 

  // auto assignments = pAss.getAssignment(); 
  // auto ranges =  std::vector<decltype(assignments.equal_range(0))>(); 
  // for(auto i = 0; i < readerComm.size() ; ++i)
    // ranges.push_back(assignments.equal_range( readerColor * readerStride + i )); 

#if 0 
  auto workLeft = true; 
  auto patLen = sizeof(uint8_t) * _taxa.size(); 
  while(workLeft)
    {
      // load an assignment for each rank into vector 
      auto assHere =  std::vector<Assignment>{}; 
      for(auto &range : ranges)
	{
	  auto &iter =range.first; 
	  if(iter != range.second )
	    {
	      assHere.push_back(iter->second); 
	      ++iter; 
	    }
	  else 
	    assHere.push_back(Assignment{0,0,0,0,0}); 	      // add a dummy 
	}

      // determine everything for scatter 
      auto displ = std::vector<int>(readerComm.getSize(),0); 
      auto lengths = std::vector<int>(); 
      for(auto ass : assHere)
	lengths.push_back(ass.width * patLen);
      for(nat i = 1 ; i < displ.size() ; ++i)
	displ[i] = displ[i-1] + lengths[i-1]; 
      auto mylen = 0;  
      auto totalLen = 0; 
      for(auto ass : assHere)
	{
	  if( int(ass.procNum) == pl.getChainComm().getRank() )
	    mylen = ass.width; 
	  totalLen += ass.width; 
	}
      
      auto allData = std::vector<uint8_t>();
      if(readerComm.getRank() == 0 )
	{
	  allData = std::vector<uint8_t>(totalLen * _taxa.size() ,0);
	  auto iter = begin(allData); 
	  for(auto ass : assHere)
	    {
	      if(ass.width != 0 )
		{
		  auto &partition = _partitions.at(ass.partId); 
		  auto pos = alnPos + std::streamoff(patLen * (partition.getLower() + ass.offset)); 
		  _in.seekg(pos); 

		  auto tp = getTimePoint(); 
		  readArray<uint8_t>( patLen * ass.width,  &(*iter)); 
		  pureIo += getDuration(tp);

		  iter += ass.width * patLen; 
		}
	    }
	  assert(iter == end(allData));
	}
      auto tp = getTimePoint() ; 
      auto myData = readerComm.scatterVariableKnownLength(allData, lengths, displ); 
      pureCommTime += getDuration(tp); 

      // now everybody can handle their data 
      if(mylen != 0 )
	{
	  auto aln = aligned_malloc<uint8_t, size_t(EXA_ALIGN)>(mylen * _taxa.size()); 
	  auto tp = getTimePoint(); 
	  for(auto j = 0 ; j < mylen; ++j)
	    {
	      auto iter = myData.data() + j * _taxa.size();
	      for(auto i = 0u ; i < _taxa.size(); ++i)
		aln[i * mylen + j ] = iter[i]; 
	    }
	  ...
	  for(auto ass : assHere)
		...
		{
		  auto tp = getTimePoint(); 
		  _partitions.at(ass.partId).setAlignment(shared_pod_ptr<uint8_t>(aln),mylen); 
  ...
  for(auto iter = range.first; iter != range.second; ++iter)
    {
      ...
      readArray<T>( assignment.width * sizeof(T) , arrForReading.data() ); 
...
std::vector<Partition> ByteFile::getPartitions()
{
  return _partitions; 
}
