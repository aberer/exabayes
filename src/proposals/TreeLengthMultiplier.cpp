...
TreeLengthMultiplier::TreeLengthMultiplier(
    double _multiplier)
...
void                    TreeLengthMultiplier::applyToState(
    TreeAln&             traln,
    PriorBelief&         prior,
    log_double&          hastings,
    Randomness&          rand,
    LikelihoodEvaluator& eval)
{
    ...
    auto   haveUniformPrior =
        dynamic_cast<UniformPrior*>(blParam->getPrior()) != nullptr;
    ...
    for (auto&b : newBranches)
    {
        ...
        b.setLength(InternalBranchLength(pow(initLength.getValue(),
                                             treeScaler)));

        if (not BoundsChecker::checkBranch(b))
            BoundsChecker::correctBranch(b);

        double realScaling = log(b.getLength().getValue()) / log(
                initLength.getValue());
        ...
        // correct?
        if (haveUniformPrior && blParam->getPrior()->getLogProb(
                ParameterContent{{tmp}}).isNegativeInfinity())
    ...
    {
        auto tmp = blParam->getPrior()->getLogProb(ParameterContent{{newTL}})
            / blParam->getPrior()->getLogProb(ParameterContent{{initTL}});
        prior.addToRatio(tmp);
    }
}


void                                TreeLengthMultiplier::resetState(
    TreeAln&traln)
{
...
void                                TreeLengthMultiplier::evaluateProposal(
...
void                                TreeLengthMultiplier::writeToCheckpointCore(
...
std::vector<nat>                    TreeLengthMultiplier::getInvalidatedNodes(
