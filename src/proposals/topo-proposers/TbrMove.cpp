...
BranchPlain                                  TbrMove::getEvalBranch(
...
std::vector<BranchPlain>                     TbrMove::getBranchesToPropose(
    const TreeAln& traln,
    MoveOptMode    mode)
{
    ...
    if (trueTbr)
    {
        ...
        {
            auto tmp = std::unordered_set<BranchPlain>(begin(result), end(
                                                           result));
    ...
    assert(std::unordered_set<BranchPlain>(begin(result), end(
                                               result)).size() ==
           result.size());
