...
ParsSprProposer::ParsSprProposer(
    double        parsWarp,
    int           depth,
    Communicator& comm)
    : _parsWarp{parsWarp}
...
static double                                        state2factor(
    nat states)
{
    static double typicalBL = 0.05;
    return log((1.0 / states) - exp(-(states / (states - 1) * typicalBL))
               / states);
}
...
ParsSprProposer::branch2parsScore                    ParsSprProposer
    ::determineScoresOfInsertions(
    TreeAln&    traln,
    ...
    int              curDepth)
{
    ...
    if (_computedInsertions.size() > 0 && _computedInsertions.find(b) !=
        _computedInsertions.end())
    ...
    {
        testInsertParsimony(traln, insertPos->next->back,       prunedTree,
                            result, curDepth);
        testInsertParsimony(traln, insertPos->next->next->back, prunedTree,
                            result, curDepth);
    }
}
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    auto prunedFromBranch = BranchPlain{
        std::get<0>(desc).getSecNode(), std::get<1>(desc).getSecNode()
    };
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    assert(weightedInsertionsBack.find(prunedFromBranch) != end(
               weightedInsertionsBack));
