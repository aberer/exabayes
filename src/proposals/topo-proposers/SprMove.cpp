    ...
    : _path{}
{
    ...
    while ((b == _path.at(0) || b.getInverted() == _path.at(0))
           ...
           (b == BranchPlain(p->number, p->back->number) || b.getInverted() ==
    ...
    const std::vector<AbstractParameter*>&params) const
{
    ...
    auto    prOuterPtr = traln.findNodePtr(
            BranchPlain(_path.getNthNodeInPath(0),
                        _path.getNthNodeInPath(1))),
            prInnerPtr = traln.findNodePtr(
            BranchPlain(_path.getNthNodeInPath(2), _path.getNthNodeInPath(1)));

    int     lastNode = _path.getNthNodeInPath(
            int(_path.getNumberOfNodes() - 1 )),
            s2LastNode = _path.getNthNodeInPath(
            int(_path.getNumberOfNodes() - 2 ));
    nodeptr s2lPtr     = traln.findNodePtr(BranchPlain(s2LastNode, lastNode)),
            lPtr = traln.findNodePtr(BranchPlain(lastNode, s2LastNode));

    nodeptr toBeInserted = prInnerPtr->back;
    nodeptr toBeInsertedPath = prOuterPtr->back;

    assert(toBeInserted->number == sTPtr->number && sTPtr->number ==
           toBeInsertedPath->number);
 ...
 */
BranchPlain                                        SprMove::getMovingSubtree(
 ...
 */
BranchPlain                                        SprMove::getEvalBranch(
    const TreeAln&traln) const
{
...
SprMove                                            SprMove::getInverseMove()
const
{
    if (_path.size() > 0)
    {
        ...
        newPath.append(BranchPlain(result._path.getNthNodeInPath(0),
                                   result._path.getNthNodeInPath(2)));

        for (nat i = 2; i < result._path.size() - 1; ++i)
            newPath.append(result._path.at(i));

        newPath.append(
            BranchPlain(result._path.getNthNodeInPath(
                            result._path.getNumberOfNodes() - 2),
                        result._path.getNthNodeInPath(1)));
        newPath.append(
            BranchPlain(
                result._path.getNthNodeInPath(
                    result._path.getNumberOfNodes() - 1),
                result._path.getNthNodeInPath(1)));
        newPath.reverse();
...
std::vector<nat>                                   SprMove::getDirtyNodes()
const
{
...
std::unordered_set<BranchPlain>                    SprMove::
...
std::vector<BranchPlain>                    SprMove::getBranchesToPropose(
    const TreeAln& traln,
    MoveOptMode    mode)
{
    ...
    case MoveOptMode::ALL_SURROUNDING:
    {
        ...
        for (auto i = 1u; i < _path.size(); ++i)
        {
            ...
            result.push_back(
                traln.getThirdBranch(_path.at(i - 1),
                                     _path.at(i)));
        }
    }
    break;
    ...
    assert(std::unordered_set<BranchPlain>(
               begin(result), end(result)).size() == result.size());
    return result;
}
...
static bool                         isSubset(
    const std::vector<nat>&listA,
    const std::vector<nat>&listB)
{
...
std::vector<nat>                    SprMove::getDirtyWrtPrevious(
    const SprMove&rhs) const
{
    ...
    if (lastCommon != -1 && not isSubset(rhs.getNodeList(), getNodeList()))
