...
void                    ExtendedTbrProposer::buildPath(
    Path&                          path,
    ...
    std::vector<AbstractParameter*>params) const
{
    ...
    while (not accepted)
    {
        ...
        path.pushToStackIfNovel(BranchPlain(currentNode->number, n->number),
                                traln);
        currentNode = n;
...
void                    ExtendedTbrProposer::determineMove(
    TreeAln&                              traln,
    ...
    auto canMove = [&](const BranchPlain&b) -> bool
        {
            ...
            else
            {
                ...
                return not (traln.isTipBranch(desc.first) && traln.isTipBranch(
                                desc.second));
            }
        };
...
BranchPlain                                  ExtendedTbrProposer::
    ...
    auto canMove = [&](const BranchPlain&b) -> bool
        {
            ...
            {
                auto desc = traln.getDescendents(b);
                result = not (traln.isTipBranch(desc.first) &&
                              traln.isTipBranch(desc.second));
                // tout << b << " with children " << std::get<0>(desc) << ","
                // << std::get<1>(desc)<< std::endl ;
            }
