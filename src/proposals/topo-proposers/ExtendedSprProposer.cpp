...
ExtendedSprProposer::ExtendedSprProposer(
    double stopProb)
    : _stopProb{stopProb}
    , _move{}
{
...
void                    ExtendedSprProposer::determineMove(
    TreeAln&                              traln,
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    auto    newBranch = BranchLengths(BranchPlain(lengthR.getPrimNode(),
                                                  q->number),
                                      lengthR.getLengths());
    ...
    while (not accepted)
    {
        ...
        modifiedPath.pushToStackIfNovel(BranchPlain(currentNode->number,
                                                    n->number), traln);

        currentNode = n;

        accepted = rand.drawRandDouble01() < _stopProb && modifiedPath.size() >
            2;
    }
    ...
    newBranch = BranchLengths(BranchPlain(lengthQ.getPrimNode(), p->number),
                              lengthQ.getLengths());
    // newBranch = lengthQ ;
    ...
    newBranch = BranchLengths(BranchPlain(lengthR.getPrimNode(), p->number),
                              lengthR.getLengths());
    // newBranch = lengthR;
    // newBranch.setSecNode(p->number);
    traln.setBranch(newBranch, params);
    ...
    modifiedPath.at(0) = traln.getThirdBranch(modifiedPath.at(0),
                                              modifiedPath.at(1));
    ...
    // TODO in principle, we can throw away the path of this proposal and use
    // the move proposal
    // move.

    auto bla = modifiedPath.at(int(modifiedPath.size() - 1));

    _move = SprMove(traln,  BranchPlain(p->number, p->back->number),
...
BranchPlain                                  ExtendedSprProposer::
