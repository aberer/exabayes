...
LikehoodSprProposer::LikehoodSprProposer(
    nat         maxStep,
    double      likeWarp,
    MoveOptMode toOpt)
    : _maxStep{maxStep}
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    auto insertions = computeLikelihoodsOfInsertions(traln, eval, prunedTree,
                                                     params);
    insertions = transformLikelihoods(insertions);

    auto myR = rand.drawRandDouble01();

    auto choice = [](const std::vector<InsertionResult>& ins, double
                     r) -> InsertionResult
        {
            for (auto&v : ins)
            {
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    auto insertsForRevMove = computeLikelihoodsOfInsertions(traln, eval,
                                                            prunedTree,
                                                            params);
    ...
    auto backInsertIter = std::find_if(begin(insertsForRevMove), end(
                                           insertsForRevMove),
...
BranchPlain                                     LikehoodSprProposer::
...
TopoMoveProposer*                               LikehoodSprProposer::clone()
const
{
...
std::vector<InsertionResult>                    LikehoodSprProposer::
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    auto moves = MoveEnumeration::getMovesFromDepthFirstSearch(traln,
                                                               prunedTree,
                                                               int(_maxStep));
    ...
    for (auto move : moves)
    {
        ...
        auto branches = move.getInverse()->getBranchesToPropose(traln,
                                                                _moveOptMode);

        if (_moveOptMode == MoveOptMode::NONE || _moveOptMode ==
            MoveOptMode::ONLY_SWITCHING)
        {
            auto setdiff = move.getDirtyWrtPrevious(prevMove);

            // tout << "SET-DIFF: " << SHOW(move) << SHOW(prevMove) <<
            // SHOW(setdiff)<< std::endl;
            for (auto n : setdiff)
        ...
        else
        {
            ...
            for (auto elem : mappedOptParams)
                optParams.insert(end(optParams), begin(elem.second), end(
                                     elem.second));

            result.emplace_back(move.getInsertBranch(),
                                bsp.getOptimalLikelihood(), optParams);
        ...
        if (not (_moveOptMode == MoveOptMode::NONE || _moveOptMode ==
                 MoveOptMode::ONLY_SWITCHING))
...
{
    auto maxElem = std::max_element(begin(result), end(result), [](const
                                                                   InsertionResult
                                                                   & a, const
                                                                   InsertionResult
                                                                   &b)
    {
        return a.getInsertionProb() < b.getInsertionProb();
    });
    ...
    for (auto&pair : result)
        tout << "DIFF " << pair.getBranch() << "\t" <<  MAX_SCI_PRECISION
             << pair.getInsertionProb()  << "\t" << SOME_FIXED_PRECISION
             << pair.getInsertionProb() << std::endl;
