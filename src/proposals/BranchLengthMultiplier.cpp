...
BranchPlain                    BranchLengthMultiplier::proposeBranch(
    const TreeAln&traln,
...
BranchPlain                    BranchLengthMultiplier::determinePrimeBranch(
    const TreeAln&traln,
    Randomness&   rand) const
{
...
void                           BranchLengthMultiplier::applyToState(
    TreeAln&             traln,
    PriorBelief&         prior,
    log_double&          hastings,
    Randomness&          rand,
    LikelihoodEvaluator& eval)
{
    ...
    // tout << MAX_SCI_PRECISION << "proposed " << newZ <<  " from " <<
    // multiplier << " and oldBranch=" << savedBranch << std::endl;
    ...
    const BranchPlain&  branchSuggestion)
{
    ...
    // tout << SOME_FIXED_PRECISION << "lnl=" << traln.getTrHandle().likelihood
    // << std::endl;
}
    ...
    autotune()
{
    ...
    double newParam = tuneParameter(_sctr.getBatch(), ratio, _multiplier,
                                    false);

    // tout << MAX_SCI_PRECISION << SHOW(_multiplier) << " -> " <<
    // SHOW(newParam) << std::endl;

    // tout << MAX_SCI_PRECISION <<   SHOW(_minTuning) << SHOW(newParam) <<
    // SHOW(_maxTuning) << std::endl;
...
AbstractProposal*                         BranchLengthMultiplier::
    clone() const
{
...
std::pair<BranchPlain,
          BranchPlain>                    BranchLengthMultiplier::
...
std::vector<nat>                          BranchLengthMultiplier::
    getInvalidatedNodes(
