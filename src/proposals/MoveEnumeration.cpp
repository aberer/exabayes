...
std::vector<SprMove>                    MoveEnumeration::
    getMovesFromDepthFirstSearch_helper(
    const TreeAln&     traln,
    ...
    bool               isFirst)
{
    ...
    if (not traln.isTipNode(insert.getSecNode()))
    {
        ...
        auto res1 = getMovesFromDepthFirstSearch_helper(
                traln, pruneBranch,
                desc.first, depth - 1,
                false);
        auto res2 = getMovesFromDepthFirstSearch_helper(
                traln, pruneBranch,
                desc.second, depth - 1,
                false);
    ...
    int                dist)
{
    ...
    auto res1 = getMovesFromDepthFirstSearch_helper(
            traln, pruneBranch,
            desc.first, dist, true);
    auto res2 = getMovesFromDepthFirstSearch_helper(
            traln, pruneBranch,
            desc.second, dist, true);
...
std::vector<SprMove>                    MoveEnumeration::getMovesForBranch(
    const TreeAln& traln,
    ...
    nat            dist)
{
    ...
    {
        auto distBranches = traln.getBranchesByDistance(
                pruneBranch, dist + 1,
                false);
        ...
        for (auto insertBranch : distBranches)
        {
            ...
            {
                // tout << "skipping " << pruneBranch << "," << insertBranch <<
                // std::endl;
                continue;
            }
...
std::vector<SprMove>                    MoveEnumeration::getAllUniqueMoves(
    const TreeAln& traln,
    nat            dist)
{
    auto result = std::vector<SprMove>{};
    ...
    for (auto pruneBranch : traln.extractBranches())
    {
        ...
        auto res2 = getMovesForBranch(
                traln,
                pruneBranch.getInverted(),
                dist + 1);
