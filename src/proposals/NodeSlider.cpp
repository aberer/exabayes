    ...
    ->std::pair<BranchPlain, BranchPlain>
{
    ...
    auto b = rand.drawRandDouble01() < 0.5 ? descendents.first :
        descendents.second;
    ...
    Randomness&       rand) const
{
    ...
    else
    {
        ...
        return rand.drawRandDouble01() < 0.5 ? descendents.first :
               descendents.second;
    }
}
    ...
    LikelihoodEvaluator& eval)
{
    ...
    otherBranch = traln.getBranch(proposeOtherBranch(oneBranch, traln, rand),
                                  param);
    ...
    if (not BoundsChecker::checkBranch(testBranch))
    {
        ...
        assert(not problemWithA); // should have been detected, when we checked
                                  // the multiplier
    }
    ...
    //   /   param->getPrior()->getLogProb( ParameterContent{{
    // oneBranch.getInterpretedLength(param) } } );
    ...
    // testBranch.getInterpretedLength(param) } } )
    //   /   param->getPrior()->getLogProb( ParameterContent{{
    // otherBranch.getInterpretedLength(param) } } );
...
void                    NodeSlider::evaluateProposal(
...
auto                    NodeSlider::getInvalidatedNodes(
