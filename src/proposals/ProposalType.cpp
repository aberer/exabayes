...
std::string                                  getConfigStringFromType(
    ProposalType p)
{
...
ProposalType                                 getTypeFromConfigString(
    std::string name)
{
...
std::vector<ProposalType>                    getSingleParameterProposalsForCategory(
    Category c)
{
    switch (c)
    {
    ...
    ProposalType p)
{
    std::unordered_map<ProposalType, bool, ProposalTypeHash> map  = {
        ...
        {ProposalType::E_TBR,  false},// dont like this one: mostly works, when
                                      // degenerated to spr
