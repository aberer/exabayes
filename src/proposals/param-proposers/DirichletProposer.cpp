...
DirichletProposer::DirichletProposer(
    double minValI,
    double maxValI,
...
std::vector<double>                    DirichletProposer::proposeValues(
    std::vector<double>oldValues,
    double             parameter,
    Randomness&        rand,
    log_double&        hastings)
{
    assert(fabs(std::accumulate(oldValues.begin(), oldValues.end(), 0.)
                - 1.0) < 1e-6);

    // sum of rates equals 1 ?
