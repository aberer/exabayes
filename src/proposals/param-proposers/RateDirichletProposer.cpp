...
RateDirichletProposer::RateDirichletProposer(
    double minValI,
    double maxValI)
...
// RateDirichletProposer::RateDirichletProposer(const RateDirichletProposer&
// rhs)
...
// OUTPUT:
// allNewRates: all rates summing up to 1, the newly proposed, but corrected
// values
// newRates : all new rates, summing up to 1 and after corection
auto                                   RateDirichletProposer::
    correctValues(
    nat                whichFreq,
    nat                numFreq,
...
std::vector<double>                    RateDirichletProposer::
    proposeValues(
    std::vector<double>allOldValues,
    double             parameter,
    Randomness&        rand,
    log_double&        hastings)
{
    ...
    auto             ratesForFreq = _rateHelper.extractSomeRates(whichFreq,
                                                                 numFreq,
                                                                 allOldValues);
    ...
    auto             alphasOld = _rateHelper.getScaledValues(ratesForFreq,
                                                             parameter);
    ...
    std::tie(newRatesForFreq, allNewValues) = correctValues(whichFreq, numFreq,
                                                            newRatesForFreq,
                                                            allOldValues);
    ...
    // scale new
    auto             alphasNew =  _rateHelper.getScaledValues(newRatesForFreq,
                                                              parameter);
    auto             backP = Density::lnDirichlet(ratesForFreq, alphasNew);

    // AbstractProposal::updateHastingsLog(hastings, backP - forP,
    // "dirichletPartial");
    hastings *= backP / forP;
