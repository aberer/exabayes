...
AbstractProposer::AbstractProposer(
    bool   tune,
    bool   tuneup,
    double minVal,
    double maxVal)
...
void                    AbstractProposer::correctAbsoluteRates(
    std::vector<double>&values) const
{
    ...
    while (prevFixed != nowFixed && valuesNotOkay)
    {
        ...
        auto numHigh = std::count_if(begin(fixedHigh), end(fixedHigh), [](bool
                                                                          elem)
        {
            return elem;
        });
        auto numLow  = std::count_if(begin(fixedLow), end(fixedLow), [](bool
                                                                        elem)
        {
            return elem;
        });

        normer = (1. - (_minVal * double(numLow)  +  _maxVal
                        * double(numHigh)))    / normer;
