...
SlidingProposer::SlidingProposer(
    double minVal,
    double maxVal,
    bool   _minMaxIsRelative)
    : AbstractProposer{true, true, minVal, maxVal}
    , minMaxIsRelative{_minMaxIsRelative}
{}


double                                 SlidingProposer::proposeOneValue(
    double     oldVal,
    double     parameter,
    Randomness&rand,
    log_double&hastings)
...
std::vector<double>                    SlidingProposer::proposeRelativeMany(
    ...
    log_double&        hastings)
{
    ...
    if (fabs(std::accumulate(oldValues.begin(), oldValues.end(), 0.) - 1.0) >
        1e-6)
    {
        std::cerr << "Danger: while proposing values,  sum was "
                  << std::accumulate(oldValues.begin(), oldValues.end(), 0.)
                  << ". values: "   << oldValues  << std::endl;
        assert(0);
    }
...
std::vector<double>                    SlidingProposer::proposeValues(
    std::vector<double>oldValues,
    double             parameter,
    Randomness&        rand,
    log_double&        hastings)
{
    if (oldValues.size() == 1)
        return {
                   proposeOneValue(oldValues[0], parameter, rand, hastings)
        };
    else
