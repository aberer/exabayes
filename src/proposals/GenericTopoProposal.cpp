...
// this is the appropriate parameter for a uniform draw in [2/3 ; 1.5] on the
// exponential scale
...
GenericTopoProposal&                   GenericTopoProposal::operator=(
...
BranchPlain                            GenericTopoProposal::
...
log_double                             GenericTopoProposal::
    ...
    LikelihoodEvaluator&           eval) const
{
    ...
    if (_useMultiplier && not _moveProposer->hasOptimizedBranches())
    {
        for (auto b : bs)
        {
            for (auto p : params)
            {
                ...
                bl.setLength(InternalBranchLength::fromAbsolute(oldLen * multi,
                                                                meanSubstRate));
                ...
                prior.addToRatio(p->getPrior()->getLogProb(
                                 ...
                                 / p->getPrior()->getLogProb(
                                     ParameterContent{{oldLen}}));
    ...
    else
    {
        auto getOptParams = [&]() -> std::vector<OptimizedParameter>
            {
                ...
                else
                {
                    ...
                    for (auto elem : bsp.getResult())
                        result.insert(end(result), begin(elem.second), end(
                                          elem.second));
                }
        ...
        for (auto&optParam : getOptParams())
        {
            ...
            auto nonConvParam =
                p->getAttributes()._nonConvTuner.getParameter();
            ...
            // tout << MAX_SCI_PRECISION << SHOW(oldLen) << " ->  " <<
            // SHOW(newLen) << SOME_FIXED_PRECISION << SHOW(hastHere)<<
            // std::endl;
        }
    }
 ...
 */
log_double                    GenericTopoProposal::assessOldBranches(
    TreeAln&                             traln,
...
void                    GenericTopoProposal::applyToState(
    TreeAln&             traln,
    PriorBelief&         prior,
    log_double&          hastings,
    Randomness&          rand,
...
void                                GenericTopoProposal::evaluateProposal(
...
void                                GenericTopoProposal::resetState(
    TreeAln&traln)
{
...
std::vector<nat>                    GenericTopoProposal::getInvalidatedNodes(
