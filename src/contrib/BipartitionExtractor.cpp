...
BipartitionExtractor::BipartitionExtractor(
...
nat                            BipartitionExtractor::getNumTreesInFile(
    std::string file) const
{
    ...
    if (result < 1)
    {
        std::cout << "Header of file " << file
                  <<
            " possibly broken. Expected list of taxa terminated by semi-colon."
                  << std::endl;
        exitFunction(-1, false);
    }
...
void                           BipartitionExtractor::extractBips(
    nat burnin)
{
    int ctr = 0;
...
void                           BipartitionExtractor::printBipartitionStatistics(
    std::string id) const
{
    assert(_uniqueBips.size() != 0);
    ...
    std::cout << "printed bipartition statistics to file " << ss.str()
              << std::endl;
}


void                           BipartitionExtractor::printBranchLengths(
    std::string id) const
{
    assert(_uniqueBips.size() != 0);
    ...
    for (auto&bipHash : _bipHashes)
    {
        for (auto&bip : bipHash)
        {
            for (auto length : bipHash.getBranchLengths(bip.first))
                blFile << _uniqueBips.at(bip.first) << "\t"  << ctr  << "\t"
                       << length << std::endl;
        }
    ...
    std::cout << "printed branch lengths associated with bipartitions to "
              << ss.str() << std::endl;
}

void                           BipartitionExtractor::printFileNames(
    std::string id) const
{
    assert(_uniqueBips.size() != 0);
    ...
    std::cout << "printed file name identifiers (for future reference) to "
              << ss.str() << std::endl;
}


void                           BipartitionExtractor::printBipartitions(
    std::string id) const
{
    assert(_uniqueBips.size() != 0);
    ...
    std::cout << "printed bipartitions and identifiers to file " << ss.str()
              << std::endl;
}


std::string                    BipartitionExtractor::bipartitionsToTreeString(
    std::vector<Bipartition>bips,
    bool                    printSupportLocal,
    bool                    printBranchLengthsLocal,
    bool                    phylipStyle) const
{
    ...
    auto&& ss = std::stringstream{};
    buildTreeRecursive(nat(bips.size() - 1), directSubBips, bips, ss,
                       printSupportLocal, printBranchLengthsLocal,
                       phylipStyle);
...
void                    BipartitionExtractor::buildTreeRecursive(
    nat                                   currentId,
    ...
    bool                                  phylipStyle) const
{
    ...
    if (currentId != bips.size() - 1)
    {
        ...
        mySupport = double(_bipHashes[0].getPresence(curBip).count())
            / totalTrees;
    ...
    else
    {
        ...
        {
            result << (isFirst ? "" : ",");
            buildTreeRecursive(childId, directSubBips, bips, result,
                               printSupportLocal, printBranchLengthsLocal,
                               phylipStyle);
            isFirst = false;
        }
    ...
    if (currentId != bips.size()  - 1)
    {
        if (printSupportLocal)
        {
            ...
            else
            {
                result << "[&prob=" << MAX_SCI_PRECISION   << mySupport
                       << ",prob(percent)=\"" << std::fixed
                       << std::setprecision(0) << mySupport * 100  << "\""
                       << "]";
            }
        }
        ...
        if (printBranchLengthsLocal)
        {
            ...
            if (phylipStyle)
                ...
                result << MAX_SCI_PRECISION << ":" << median
                       << "[&length_mean=" << mean << ",length_median="
                       << median
                       << ",length_95%HDP={" << p5 << "," << p95 << "}]";
        }
    }
    else
        result << ";";
...
template void                    BipartitionExtractor::extractBips<true>(
    nat burnin);
template void                    BipartitionExtractor::extractBips<false>(
    nat burnin);
