...
auto                    ConsensusTree::getRefinedConsensusTree(
...
auto                    ConsensusTree::getConsensusTreeString(
    ...
    ->std::string
{
    ...
    auto sortBySecond = [](const std::pair<Bipartition, nat>&elemA, const
                           std::pair<Bipartition, nat>&elemB)
        {
            return elemA.second < elemB.second;
        };

    auto bip2occVect = std::vector<std::pair<Bipartition, nat> >{
        bip2Occ.begin(), bip2Occ.end()
    };
...
auto                    ConsensusTree::getTreeHeader() const
    ->std::string
{
...
auto                    ConsensusTree::getType() const
    ->std::string
{
    ...
    {
        if (std::fabs(_threshold - 1.)  <
            std::numeric_limits<double>::epsilon())
            type = "ConsensusStrict";
        else if (std::fabs(_threshold - 0.5) <
                 std::numeric_limits<double>::epsilon())
            type = "ConsensusMajorityRule";
        else
        {
            auto&&ss = std::ostringstream{};
            ss << "ConsensusThreshold" << std::setprecision(0) << std::fixed
               << nat(_threshold * 100.);
            type = ss.str();
        }
    }
