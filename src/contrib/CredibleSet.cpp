...
void                    CredibleSet::printCredibleSet(
    std::string filename,
    double      thresh)
{
    ...
    auto        sortedBipOcc = std::vector<std::pair<Bipartition,
                                                     Bipartition> >{};
    ...
    auto        equalFun = [](const std::vector<nat>&treeA,  const
                              std::vector<nat>&treeB)  -> bool
        {
            bool equal = true;
    ...
    auto        trees =  std::unordered_map<std::vector<nat>, nat,
                                            decltype(hashFun),
                                            decltype(equalFun)>{
        0, hashFun, equalFun
    };

    nat         totalTrees = _bipEx.getBipartitionHashes().at(
            0).getTreesAdded();
    ...
    for (nat i = 0; i < totalTrees; ++i)
    {
        ...
        // std::cout << "expected " << oneTree.size() << " , got " << numBip <<
        // std::endl;
    ...
    std::sort(treeAndOcc.begin(), treeAndOcc.end(), [](const
                                                       std::pair<std::vector<
                                                                     nat>,
                                                                 nat>&elemA,
                                                       const std::pair<std::
    ...
    for (auto&elem : treeAndOcc)
    {
        ...
        auto  result = _bipEx.bipartitionsToTreeString(bips, false, false,
                                                       true);
    ...
    std::sort(begin(treeStrings), end(treeStrings),
              [](const std::pair<std::string, nat>&elemA, const
                 std::pair<std::string, nat>&elemB)
    {
        return elemA.second > elemB.second;
    });
    ...
    std::cout << "printed the " << thresh *  100  <<  "% credible set to "
              << filename << std::endl;
}
