...
void                    SplitFreqAssessor::extractBips(
...
auto                    SplitFreqAssessor::computeAsdsfNew(
    double ignoreFreq)
...
nat                    SplitFreqAssessor::getNumTreeAvailable(
    string fileName)
{
    string   whitespace = " \t";
    ...
    while (getline(infile, line))
    {
        ...
        else if (foundTaxaStart) // check if all taxa are there and have the
                                 // appropriate number
        {
            if (cleanline[cleanline.size() - 1] == ';')
                foundTreeStart = true;
            ...
            {
                std::cout << "expected " << name << " but got " << _taxa.at(
                    index) << std::endl;
