...
void                            newviewParsimonyIterativeFast(
    ...
    partitionList* pr);
// ____________________________________________________________________________
unsigned int                    evaluateParsimonyIterativeFast(
    pllInstance*  tr,
    partitionList*pr);
// ____________________________________________________________________________
void                            buildSimpleTree(
    ...
    int           ir);
// ____________________________________________________________________________
void                            makePermutationFast(
    int*        perm,
    int         n,
    pllInstance*tr);
// ____________________________________________________________________________
void                            computeTraversalInfoParsimony(
    nodeptr p,
    int*    ti,
    int*    counter,
    int     maxTips,
...
void                            TreeRandomizer::stepwiseAddition(
    pllInstance*   tr,
    partitionList* pr,
    nodeptr        p,
    nodeptr        q,
...
void                    TreeRandomizer::createStepwiseAdditionParsimonyTree(
    TreeAln&       traln,
    ParallelSetup& pl)
{
    ...
    while (tr->ntips < tr->mxtips)
    {
        ...
        // std::cout <<  SyncOut() << "bestPars: "<< tr->bestParsimony <<
        // std::endl;
...
void                           TreeRandomizer::createParsimonyTree(
    TreeAln&       traln,
    Randomness&    rand,
    ParallelSetup& pl)
{
    ...
    Randomness& rand)
{
    ...
    for (nat i = 4; i < traln.getNumberOfTaxa() + 1;  ++i)
    {
        ...
        auto p1 = traln.findNodePtr(drawBranchUniform_helper(traln, rand, i
                                                             - 1)),
             p2 = p1->back;
...
BranchPlain                    TreeRandomizer::drawInnerBranchUniform(
    const TreeAln& traln,
    Randomness&    rand)
{
    bool    acc = false;
    ...
    while (not acc)
    {
        ...
        acc = numTips == 0 || rand.drawRandDouble01() <  (3.
                                                          - double(numTips))
            / 3.;
    }
...
nat                            TreeRandomizer::drawInnerNode(
    const TreeAln& traln,
    Randomness&    rand)
{
...
// draw a branch that has an inner node as primary node
//
// => equals draw subtree uniformly
BranchPlain                    TreeRandomizer::drawBranchWithInnerNode(
    const TreeAln& traln,
    Randomness&    rand)
{
...
BranchPlain                    TreeRandomizer::drawBranchUniform(
    const TreeAln& traln,
    Randomness&    rand)
{
...
BranchPlain                    TreeRandomizer::drawBranchUniform_helper(
    const TreeAln&traln,
    Randomness&   rand,
    nat           curNumTax)
{
    ...
    if (r <= 0.75)      // draw an inner node
        randId = 1 + traln.getTrHandle().mxtips + rand.drawIntegerClosed(
                curNumTax - 3);
    else            // draw a tip
