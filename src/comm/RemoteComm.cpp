...
RemoteComm&                                       RemoteComm::operator=(
    RemoteComm rhs)
{
    swap(rhs, *this);
    return *this;
...
template auto                                     RemoteComm
    ...
    int                      root)->std::vector<unsigned int>;
// template auto RemoteComm::broadcast(std::vector<unsigned long> array, int
// root)  -> std::vector<unsigned long>;
...
template uint64_t                                 RemoteComm
    ::broadcastVar(
    uint64_t var,
    int      root);
template uint32_t                                 RemoteComm
    ::broadcastVar(
    uint32_t var,
    int      root);
...
template std::vector<char>                        RemoteComm
    ::gather(
    std::vector<char>myData,
    nat              root);
template std::vector<int>                         RemoteComm
    ::gather(
...
template std::vector<double>                      RemoteComm
    ::allReduce(
    std::vector<double>myValues);
template std::vector<uint32_t>                    RemoteComm
    ::allReduce(
    std::vector<uint32_t>myValues);
template std::vector<uint8_t>                     RemoteComm
    ::allReduce(
...
auto                                              RemoteComm
    ...
    ->std::vector<T>
{
    return _impl->scatterVariableKnownLength(allData, countsPerProc,
                                             displPerProc, root);
}
template auto                                     RemoteComm
    ::scatterVariableKnownLength(
...
template auto                                     RemoteComm
    ::scatterVariableKnownLength(
    std::vector<uint32_t>allData,
    std::vector<int>&    countsPerProc,
    std::vector<int>&    displPerProc,
    int                  root)->std::vector<uint32_t>;
template auto                                     RemoteComm
    ::scatterVariableKnownLength(
    std::vector<uint16_t>allData,
    std::vector<int>&    countsPerProc,
    std::vector<int>&    displPerProc,
    int                  root)->std::vector<uint16_t>;
template auto                                     RemoteComm
    ::scatterVariableKnownLength(
    std::vector<uint8_t>allData,
    std::vector<int>&   countsPerProc,
    std::vector<int>&   displPerProc,
    int                 root)->std::vector<uint8_t>;
...
auto                                              RemoteComm
    ...
    ->std::vector<T>
{
    return _impl->gatherVariableKnownLength(myData, countsPerProc,
                                            displPerProc, root);
}
