...
std::ostream&                   operator<<(
    ...
    const SwapElem&elem)
{
    out << "gen=" << elem._gen << " " <<  elem._idA << "<->"  <<  elem._idB
        <<  " " << elem._r;
    return out;
}


bool                            SwapElem::onlyOneIsMine(
    ParallelSetup& pl,
    nat            runid) const
{
    auto oneIsMine = (not pl.isMyChain(runid, _idA)) != (not  pl.isMyChain(
                                                             runid, _idB));
    return oneIsMine;
}


bool                            SwapElem::bothAreMine(
    ParallelSetup& pl,
    nat            runid) const
{
