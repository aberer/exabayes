...
std::tuple<size_t,
           size_t>                    IncompleteMesh
    ::
    getElementsPerDimension(
    size_t total,
    size_t dimSize) const
{
    ...
    assert(numMoreProc * (cartProcs + 1)  +  (dimSize - numMoreProc)
           * cartProcs == total);
    ...
    size_t dimSize) const
{
    ...
    return (rank < (dimSize - numWithFewProcs) * (procs + 1)) ? procs + 1  :
           procs;
}
    ...
    nat rank) const
{
    ...
    auto numProcMyChain = getProcsInMyDim(rankInRun, numProcMyRun,
                                          _chainDimSize);
...
nat                    IncompleteMesh::getNumRanksInDim(
    nat runBatchId,
    nat chainBatchId) const
{
    ...
    {
        otherRank = getRankFromCoordinates({{runBatchId, chainBatchId + 1,
                                             0}});
    }
