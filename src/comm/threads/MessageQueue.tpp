...
template<typename T>
void                                                 MessageQueue::produce(
    std::vector<T>         msg,
    const std::vector<int>&forWhom)
{
    auto tmpMsg = std::vector<byte>(reinterpret_cast<byte*>(msg.data()),
                                    reinterpret_cast<byte*>(msg.data()
                                                            + msg.size()));
...
template<typename T>
std::tuple<bool, std::vector<T> >                    MessageQueue::consume(
    int threadId)
{
