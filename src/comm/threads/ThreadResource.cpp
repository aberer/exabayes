  ...
  : _threadsAreReleased(false)
{
  ...
  if(haveThreadSupport)
	...
	std::cout << "You wanted to start threads using -T. However, " << PROGRAM_NAME << " could not detect thread support. Will attempt to continue without threads."  << std::endl; 
      auto highestRank = 0; 
      _tid2rank[MY_TID] = highestRank++; 
    }
}
...
void ThreadResource::pinThreads(int numProcPerNode, int remoteRank)
{
  ...
  // std::cout << SyncOut() << "pinnig thread  " << _tid2rank[MY_TID ] << " of process "  << remoteRank << " to core "  << pinTo << std::endl; 
  ...
  if(result != 0)
    {
      std::cout << SyncOut() << "Problem with pinning! Probably the number of processes and threads\n"
		<< "started on this machine exceeds the number of available cores. Thread\n"
		<< "pinning is disabled. In the worst case," << PROGRAM_NAME << " will run substantially slower (use a tool like htop to monitor,\n"
