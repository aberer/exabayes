    ...
    , _tid2rank{}
{
    ...
    {
        // this starts the threads and makes them wait in the function
        // threadStart
        int highestRank = 0;
        ...
        if (cl.getNumThreads() > 1)
            std::cout << "You wanted to start threads using -T. However, "
                      << PROGRAM_NAME << " could not detect thread support. "
                               " Will attempt to continue without threads."
                      << std::endl;
...
ThreadResource&                   ThreadResource::operator=(
    ThreadResource rhs)
{
    swap(*this, rhs);
    return *this;
...
void                              ThreadResource::pinThreads(
    int numProcPerNode,
    int remoteRank)
{
#ifndef __APPLE__
    ...
    if (result != 0)
    {
        std::cout << SyncOut() << "Problem with pinning! Probably the number "
                                  ...
                                  "Thread\n"
                                  "pinning is disabled. In the worst case,"
                  << PROGRAM_NAME
                  <<
...
void                              ThreadResource::threadStart(
    CommandLine&   cl,
    ParallelSetup* pl)
{
