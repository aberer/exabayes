/*
 * all communicators (impls or wrapper classes) have to implement these
 * declarations
 */
...
template<typename T>
auto                              gatherVariableLength(
    std::vector<T>myData,
    int           root = 0)
    ->std::vector<T>;

template<typename T>
auto                              gatherVariableKnownLength(
    std::vector<T>   myData,
...
template<typename T>
auto                              scatterVariableKnownLength(
