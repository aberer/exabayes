...
ParallelSetup::ParallelSetup(
    ...
    , _mesh(_globalComm.size(), cl.getNumRunParallel(),
        cl.getNumChainsParallel())
{}


ParallelSetup&                   ParallelSetup::operator=(
    ParallelSetup rhs)
{
...
void                             ParallelSetup::initializeRemoteComm(
    int   argc,
    char**argv)
{
...
void                             ParallelSetup::warn()
{
    ...
    if (numWorkers <  _mesh.getRunDimSize() * _mesh.getChainDimSize())
    {
        if (_globalComm.getRank() == 0)
            std::cout << "\nError: You attempted to run "
                      << _mesh.getRunDimSize()
                      << " runs in parallel for which "
                      <<  _mesh.getChainDimSize()
                      <<
                " coupled chains should be run in parallel. This requires at least "
                      << (_mesh.getRunDimSize() * _mesh.getChainDimSize())
                      <<
                " processes (in the best case a multiple of this number). "
                      << PROGRAM_NAME << " was started with "
                      << _globalComm.size()
                      << " processes though.\n" <<  std::endl;
...
void                             ParallelSetup::initialize()
{
    ...
    if (_threadResource.getNumThreads() > 1 &&
        not _globalComm.haveThreadSupport())
    {
        if (isYggdrasil)
        {
            if (isGlobalMaster())
                std::cout << "You instructed " << PROGRAM_NAME
                          << " to use threads. However, " << PROGRAM_NAME
                          <<
                    " has not been compiled with thread support. Most likely, you do not have pthreads installed. Most likely, you still can use this executable without threads (i.e., without -T x). "
                          << std::endl;
        ...
        else
        {
            if (isGlobalMaster())
                std::cout << "You instructed " << PROGRAM_NAME
                          << " to use threads. However either " << PROGRAM_NAME
                          <<
                    " has not been compiled with thread support (e.g., because the pthreads is not installed) or your MPI implementation does not support multi-threading."
                          << std::endl;
...
std::string                    ParallelSetup::printLoadBalance(
    const TreeAln& traln,
    nat            numRun,
    nat            numCoupled)
{
    ...
    if (sumA == 0)
    {
        ss << std::endl
           <<
            "WARNING: there is a process that does not evaluate any portion of the\n"
           << "data. You could decrease the number of processes."
           << std::endl;
    }
    ...
    for (auto&run : runs)
    {
        for (nat i = 0; i < run.getChains().size(); ++i)
        {
            ...
            if (isMyChain(run.getRunid(), i))
            {
                ...
                serialized.insert(end(serialized), begin(chainSer), end(
                                      chainSer));
            }
        }
    }
    ...
    if (( commFlags& CommFlag::SWAP ) != CommFlag::NOTHING)
    {
        ...
        for (auto&run : runs)
        {
            if (isMyRun(run.getRunid()))
            {
                ...
                serialized.insert(end(serialized), begin(asString), end(
                                      asString));
            }
        }
...
// TODO i guess we communicate the swap matrix mutliple times, this could be
// saved
void                    ParallelSetup
    ::
    synchronizeChainsAtMaster(
    std::vector<CoupledChains>& runs,
    CommFlag                    commFlags)
{
#if 0
    ...
    std::tie(lengthOfChainStream, myData) = serializeAllChains(runs,
                                                               commFlags);
    ...
    auto allData = _globalComm.gatherVariableKnownLength(
            countsPerProc[_globalComm.getRank()] == 0 ? std::vector<char>{} :
            myData,
            countsPerProc, displPerProc);
                ...
                for (nat i = 0; i < run.getChains().size(); ++i)
                {
                    if (isMyChain(rankOfChainLeader, run.getRunid(), i))
                    {
                        ...
                        auto&&iss =  std::istringstream{
                            std::string(streamIter, streamIter
                                        + lengthOfChainStream)
                        };
            ...
            if (( commFlags& CommFlag::SWAP ) != CommFlag::NOTHING)
            {
                for (auto&run : runs)
                {
                    if (isMyRun(rankOfChainLeader, run.getRunid()))
                    {
                        auto&& iss = std::istringstream{
                            std::string{
                                streamIter, streamIter + lengthOfSwap
                            }
                        };
...
bool                                  ParallelSetup::isMyChain(
    nat gRank,
    nat runid,
    nat chainIndex) const
{
    auto hisCoords = _mesh.getCoordinates(gRank);
    return isMyRun(gRank, runid) &&  (chainIndex % getChainsParallel()) ==
           hisCoords[1];
}
...
std::string                           ParallelSetup::sendRecvChain(
...
void                                  ParallelSetup::blockingPrint(
    Communicator&comm,
    std::string  ss)
{
...
bool                                  ParallelSetup::isRunLeader(
    nat gRank) const
{
    auto coords = _mesh.getCoordinates(_globalComm.getRank());
...
bool                                  ParallelSetup::swapIsLocal(
    nat chainIdA,
    nat chainIdB,
    nat runId) const
{
...
nat                                   ParallelSetup::chainIdToLeaderRank(
    nat runId,
    nat chainId) const
{
...
int                                   ParallelSetup::getRankInRun(
    std::array<nat, 3>coords) const
{
    auto localRootCoords = std::array<nat, 3>{{
                                                  coords[0], 0, 0
                                              }};
    return _mesh.getRankFromCoordinates(coords)
           - _mesh.getRankFromCoordinates(localRootCoords);
}


int                                   ParallelSetup::getColorInChain(
    std::array<nat, 3>coords) const
{
...
void                                  ParallelSetup::pinThreads()
{
    ...
    if (_threadResource.getNumThreads() > 1)
    {
        ...
        _threadResource.pinThreads(procsPerNode,
                                   _globalComm.getRemoteComm().getRank());
    }
}
