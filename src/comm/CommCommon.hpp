...
#define FOR_EACH(action, TOK, CLASS, ...)                 \
    GET_MACRO(__VA_ARGS__, FE_5, FE_4, FE_3, FE_2, FE_1)(action, TOK, CLASS, \
                                                         __VA_ARGS__)
...
#define SCATTER_VARIABLE_KNOWN_LENGTH(TYPE) \
    scatterVariableKnownLength(std::vector<TYPE>&myData, \
                               std::vector<int>&countsPerProc, \
                               std::vector<int>&displPerProc,  int \
                               root) const->std::vector<TYPE>
