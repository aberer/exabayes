...
bool                                 LocalSwap::allHaveReceived(
    ParallelSetup& pl)
{
    ...
    {
        auto wasThere = false;
        std::tie(wasThere, _dataReceived) = localComm.readAsyncMessage<char>(
                int(tag), runBatch);
    ...
    nat              runid)
{
    ...
    if (pl.isChainLeader())
        pl.getChainComm().getLocalComm().postAsyncMessage(
            myChainSer, int(tag), runBatch);
}
