...
void                            Communicator::createSendRequest(
...
void                            Communicator::createRecvRequest(
    int          src,
    int          tag,
    nat          length,
    CommRequest& req)
...
{
    return _remoteComm.getRank()  * int(_localComm.size())
           + _localComm.getRank();
}
...
{
    // std::cout << "called split with " << color << " and "  << rank <<
    // std::endl;

    Communicator result(_localComm.getTid2Ranking());
...
int                             Communicator::mapToLocalRank(
    int rank) const
{
...
int                             Communicator::mapToRemoteRank(
    int rank) const
{
...
void                            Communicator::initWithMaxChains(
    size_t numChains,
    size_t numThreadsChecking)
{
    if (_localComm.getColor() == 0 && _localComm.getRank() == 0)
        _localComm.initializeAsyncQueue(nat(numThreadsChecking), size_t(
                                            AbstractPendingSwap::cantorPair(
                                                numChains, numChains) * 2));
}
