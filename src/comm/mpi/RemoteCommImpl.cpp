...
RemoteComm::Impl::Impl(
    const RemoteComm::Impl& rhs)
    : _comm{MPI_COMM_WORLD}
{
    MPI_Comm_dup(rhs._comm, &_comm);
}

RemoteComm::Impl&                    RemoteComm::Impl::operator=(
    RemoteComm::Impl rhs)
{
    swap(*this, rhs);
    return *this;
...
RemoteComm::Impl                     RemoteComm::Impl::split(
...
template<>
bool                                 RemoteComm::Impl::broadcastVar(
    bool var,
    int  root)
{
    char elem = var ? 1 : 0;
...
template<typename T>
T                                    RemoteComm::Impl::broadcastVar(
    T   var,
    int root)
{
...
template uint64_t                    RemoteComm::Impl::broadcastVar(
    uint64_t var,
    int      root);
template uint32_t                    RemoteComm::Impl::broadcastVar(
    uint32_t var,
    int      root);


void                                 RemoteComm::Impl::createSendRequest(
    ...
    CommRequest&     req)
{
    ...
    MPI_Isend((void*)impl._array.data(), int(impl._array.size()), MPI_CHAR,
              dest, tag, _comm, &(impl._req));
}


void                                 RemoteComm::Impl::createRecvRequest(
    int         src,
    int         tag,
    nat         length,
    CommRequest&req)
{
    ...
    assert(impl._array.size() != 0);
    MPI_Irecv(impl._array.data(), int(impl._array.size()), MPI_CHAR, src, tag,
              _comm, &impl._req);
}
...
nat                                  RemoteComm::Impl::getNumberOfPhysicalNodes()
{
    ...
    if (getRank() == 0)
    {
        ...
        for (auto i = 0; i < int(size()); ++i)
            uniqStr.insert(std::string(begin(everything) + (i
                                                            *
                                                            MPI_MAX_PROCESSOR_NAME),
                                       begin(everything) + ((i + 1)
                                                            *
                                                            MPI_MAX_PROCESSOR_NAME)));
    ...
    char**argv)
{
    ...
    if (numThreads > 1)
    {
        ...
        {
            std::cout << "You called the mpi version of " << PROGRAM_NAME
                      <<
                " with threads (-T x).  However, the MPI implementation you installed\n"
                      <<
            ...
            std::cout << "Initialized multi-threaded MPI environment"
                      << std::endl;
    }
    else
        MPI_Init(&argc, &argv);
...
bool                    RemoteComm::Impl::haveThreadSupport()
const
{
    int provided  = 0;
