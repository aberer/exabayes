...
uint64_t                             PendingSwap::Impl::createTag(
    int rank) const
{
    ...
    nat              runid)
{
    ...
    auto numInMyBatch = pl.getMesh().getNumRanksInDim(nat(ourRunBatch), nat(
                                                          myBatch));
    auto numInRemoteBatch = pl.getMesh().getNumRanksInDim(nat(ourRunBatch),
                                                          nat(remoteBatch));
    ...
    // auto srcRankInRun =  ;
    auto remRank =  pl.getRunComm().mapToRemoteRank(pl.getRankInRun({{nat(
                                                                          ourRunBatch),
                                                                      nat(
                                                                          remoteBatch),
                                                                      nat(
    ...
    pl.getRunComm().createRecvRequest(int(remRank), int(tag), nat(
                                          myChainSer.size()), _recvReq);
    ...
    for (nat i = 0; i < numInRemoteBatch; ++i)
    {
        ...
        {
            auto  hisRemoteRank = pl.getRunComm().mapToRemoteRank(
                    pl.getRankInRun({{nat(ourRunBatch), nat(remoteBatch), nat(
                                          i)}}));

            _sentReqs.push_back(CommRequest());
            auto&&elem = _sentReqs.back();
            pl.getRunComm().createSendRequest(myChainSer, hisRemoteRank, nat(
                                                  tag), elem);
        }
    }
}
    ...
    ParallelSetup& pl)
{
    ...
    // std::cout << SyncOut()  << pl.getGlobalComm().getRank() << " received by
    // "<< toReduce << std::endl;
