...
template<typename T>
std::vector<T>                    Communicator::broadcast(
    std::vector<T>array,
    int           root)
{
...
template<typename T>
std::vector<T>                    Communicator::gather(
    std::vector<T>myData,
    nat           root)
{
    assert(0);
...
template<typename T>
std::vector<T>                    Communicator::gatherVariableLength(
    std::vector<T>myData,
    int           root)
{
    auto localRoot  = mapToLocalRank(root);
...
template<typename T>
std::vector<T>                    Communicator::allReduce(
    std::vector<T>myValues)
{
    // std::cout << SyncOut()<< _localComm.getColor() << "," <<
    // _localComm.getRank()  << "before: " << myValues << std::endl;
    ...
    // std::cout << SyncOut()<< _localComm.getColor() << "," <<
    // _localComm.getRank()  << "after: " << myValues << std::endl;
...
template<typename T>
std::vector<T>                    Communicator::gatherVariableKnownLength(
    std::vector<T>   myData,
    ...
    int              root)
{
    ...
    for (auto i = _localComm.size() * _remoteComm.getRank(); i <
         _localComm.size() * (_remoteComm.getRank() + 1); ++i)
    ...
    myData = _localComm.gatherVariableKnownLength(myData, countsLocal,
                                                  displLocal, localRoot);
    ...
    if (localRoot  == _localComm.getRank())
        myData = _remoteComm.gatherVariableKnownLength(myData, countsRemote,
                                                       displRemote,
                                                       remoteRoot);
...
template<typename T>
std::vector<T>                    Communicator::scatterVariableKnownLength(
    std::vector<T>   myData,
    std::vector<int>&countsPerProc,
    std::vector<int>&displPerProc,
    int              root)
{
    ...
    if (localRoot == _localComm.getRank())
        myData = _remoteComm.scatterVariableKnownLength(myData, countsRemote,
                                                        displRemote,
                                                        remoteRoot);
    ...
    myData = _localComm.scatterVariableKnownLength(myData, countsLocal,
                                                   displLocal, localRoot);
