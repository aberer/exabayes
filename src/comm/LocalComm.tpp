...
template<typename T>
std::vector<T>                    LocalComm::scatterVariableKnownLength(
    std::vector<T>   allData,
    std::vector<int>&countsPerProc,
    std::vector<int>&displPerProc,
    int              root)
{
    // does not need to be terrible efficient...(binary tree of course would be
    // better)
    ...
    if (myRank == root)
    {
        for (auto i = 0; i < int(size()); ++i)
        {
            ...
            auto toSend = std::vector<T>(begin(allData) + displPerProc[i],
                                         begin(allData) + displPerProc[i]
                                         + countsPerProc[i]);
            _newMessages[myIdx][i].produce(toSend);
        }

        result.insert(end(result), begin(allData) + displPerProc[myRank],
                      begin(allData) + displPerProc[myRank]
                      + countsPerProc[myRank]);
    }
    else
    {
        bool found = false;
        ...
        while (not found)
            std::tie(found, result) =
                _newMessages[rootIdx][myRank].consume<T>(myRank);
    }
...
template<typename T>
std::vector<T>                                 LocalComm::
    gatherVariableKnownLength(
    std::vector<T>   myData,
    std::vector<int>&countsPerProc,
    std::vector<int>&displPerProc,
    int              root)
{
    ...
    DATA_COMBINE_FUN gatherer = [](std::vector<T>& acc, typename
                                   std::vector<T>::const_iterator beginDon,
                                   typename
                                   std::vector<T>::const_iterator endDon)
        {
    ...
    int           root)
{
    ...
    {
        DATA_COMBINE_FUN reducer = [](std::vector<T>&acc,  typename
                                      std::vector<T>::const_iterator
                                      beginDonator, typename
                                      std::vector<T>::const_iterator
                                      endDonator)
            {
                std::transform(
...
template<typename T>
std::vector<T>                                 LocalComm::
    gatherVariableLength(
    std::vector<T>myData,
    int           root)
{
    assert(root == 0);
    DATA_COMBINE_FUN gatherer = [](std::vector<T>&acc, typename
                                   std::vector<T>::const_iterator beginDonator,
                                   typename
                                   std::vector<T>::const_iterator endDonator)
        {
    ...
    int           root)
{
    ...
    if (myRank != root)
    {
        ...
        while (not gotMsg)
            std::tie(gotMsg, msg) = hisMsg[myRank].consume<T>(myRank); // must
...
template<typename T>
std::vector<T>                    LocalComm::commTreeUpAsync(
    std::vector<T>   data,
    int              root,
...
template<typename T>
std::vector<T>                    LocalComm::commTreeUp(
    std::vector<T>   data,
    int              root,
    DATA_COMBINE_FUN fun)
{
         ...
         offset *= 2)
    {
        if ((myRank % (offset * 2)) == 0)
        {
            ...
            // be the real rank to avoid confusion
