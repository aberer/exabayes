...
void                                  newviewParsimony(
    pllInstance*  tr,
    partitionList*pr,
    nodeptr       p);
// ________________________________________________________________________
unsigned int                          evaluateParsimony(
    pllInstance*  tr,
    partitionList*pr,
    nodeptr       p,
    boolean       full);
...
void                                  ParsimonyEvaluator::disorientNode(
    nodeptr p)
{
    if (p->xPars == 1)
    {
...
void                                  ParsimonyEvaluator::evaluateSubtree(
    TreeAln&traln,
    nodeptr p)
{
    newviewParsimony(&(traln.getTrHandle()), &(traln.getPartitionsHandle()),
                     p);
}


std::array<nat, 3>                    ParsimonyEvaluator::evaluate(
    TreeAln&traln,
    nodeptr p,
    bool    fullTraversal)
{
    auto result = std::array<parsimonyNumber, 3>{{
                                                     0, 0, 0
                                                 }};
    ...
    evaluateParsimony(&tHandle, &pHandle, p, fullTraversal ? PLL_TRUE :
                      PLL_FALSE);
