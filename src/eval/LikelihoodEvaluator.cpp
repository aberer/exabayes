...
LikelihoodEvaluator&                   LikelihoodEvaluator::operator=(
    LikelihoodEvaluator rhs)
{
    swap(*this, rhs);
    return *this;
...
void                                   LikelihoodEvaluator::updatePartials(
    TreeAln&traln,
    nodeptr p)
{
    ...
    /* the first entry of the traversal descriptor is always reserved for
     * evaluate or branch length optimization calls,
     * hence we start filling the array at the second entry with index one.
     * This is not very nice and should be fixed
     * at some point */

    tr->td[0].count = 0;

    /* compute the traversal descriptor, which will include
     * nodes-that-need-update descending the subtree  p */
    computeTraversal(tr, p, PLL_TRUE, pr->perGeneBranchLengths ?
                     pr->numberOfPartitions : 1);

    /* the traversal descriptor has been recomputed -> not sure if it really
     * always changes, something to
            ...
            for (nat model = 0; model < traln.getNumberOfPartitions(); ++model)
            {
                ...
                if (tr->td[0].executeModel[model] && width > 0)
                {
                    ...
                    if (tr->saveMemory)
                    {
                        ...
                        auto  x1_gap =
                            &(partition.gapVector[tInfo.qNumber
                                                  * partition.
                                                      gapVectorLength
                              ]);
                        auto  x2_gap =
                            &(partition.gapVector[tInfo.rNumber
                                                  * partition.
                                                      gapVectorLength
                              ]);
                        auto  x3_gap =
                            &(partition.gapVector[tInfo.pNumber
                                                  * partition.
                                                      gapVectorLength
                              ]);

                        for (j = 0; j <
                             (size_t)pr->partitionData[model]->gapVectorLength;
                             j++)
                        {
                            ...
                            // std::cout << SyncOut() << "set bits is " <<
                            // setBits << std::endl;
                        }

                        requiredLength = (width - setBits)
                            * Partition::maxCategories * partition.states
                            * sizeof(double);
                    }
                    else
                    {
                        // wtf is virtual width?
                        requiredLength  = size_t(Partition::maxCategories)
                            *  virtual_width(int(width))   * partition.states
                            * sizeof(double);
                    }

                    auto x3_start = partition.xVector[tInfo.pNumber
                                                      - tr->mxtips - 1];
                    auto availableLength =
                        partition.xSpaceVector[(tInfo.pNumber - tr->mxtips
                                                - 1)];
...
void                    LikelihoodEvaluator::evaluateLikelihood(
    TreeAln&    traln,
    BranchPlain branch,
    bool        fullTraversal,
    bool        isDebugEval)
{
    ...
    {
        if (tr->td[0].count != 1)
            tout << "danger " << SHOW(tr->td[0].count) << " but expected 1"
                 << std::endl;
    }
...
bool                    LikelihoodEvaluator::applyDirtynessToSubtree(
    TreeAln&          traln,
    nat               partId,
    const BranchPlain&branch)
{
    ...
    nat   id =  ArrayOrientation::nodeNum2ArrayNum(branch.getPrimNode(),
...
// #if 1
    bool  isClean = partition.xSpaceVector[id] != 0  &&
        _arrayOrientation.isCorrectNew(traln, partId, p);
// #else
//   bool isClean = partition.xSpaceVector[id] != 0  &&
// _arrayOrientation.isCorrect(partId, id, p->back->number);
// #endif
...
void                    LikelihoodEvaluator::evaluate(
    TreeAln&          traln,
    const BranchPlain&root,
    bool              fullTraversal)
{
    // tout << "evaluate with root " << root << " and " << ( fullTraversal ?
    // "full" : "partial" ) << std::endl;
...
void                    LikelihoodEvaluator::evaluateSubtrees(
    TreeAln&                traln,
    const BranchPlain&      root,
...
void                    LikelihoodEvaluator::evaluatePartitionsDry(
    TreeAln&                traln,
    const BranchPlain&      root,
    const std::vector<nat>& partitions)
{
...
void                    LikelihoodEvaluator::evaluatePartitionsWithRoot(
    TreeAln&                traln,
    const BranchPlain&      root,
    const std::vector<nat>& partitions,
    bool                    fullTraversal)
{
    // tout << "evaluatePartitions with root " << root << " and " << (
    // fullTraversal ? "full" : "partial" )  << " and partitons " << partitions
    // << std::endl;
    auto numPart = traln.getNumberOfPartitions();
    ...
    traln.setLikelihood(std::accumulate(begin(perPartitionLH), end(
                                            perPartitionLH),
                                        log_double::fromAbs(1.0),
                                        std::multiplies<log_double>()));
...
/**
 *  NOTE: the fullTraversal parameter is only there because of the searching
 * (HACK)
 */
void                    LikelihoodEvaluator::evalSubtree(
    TreeAln&          traln,
    nat               partId,
    ...
    bool              fullTraversal)
{
    ...
    _arrayPolicy->prepareForEvaluation(traln, evalBranch, partId,
                                       _arrayOrientation, *_arrayReservoir);
...
void                    LikelihoodEvaluator::disorientDebugHelper(
    TreeAln&           traln,
    const BranchPlain& root)
{
...
void                    LikelihoodEvaluator::disorientDebug(
    TreeAln&           traln,
    const BranchPlain& root)
{
...
void                    LikelihoodEvaluator::expensiveVerify(
    TreeAln&    traln,
    BranchPlain root,
    log_double  toVerify)
{
    ...
    *_debugTralnPtr = traln;
    ...
    if (fabs(log_double(verifiedLnl / toVerify).toAbs())  - 1.0  >
        ACCEPTED_LIKELIHOOD_EPS)
    {
        ...
        tout
            <<
            "WARNING: found in expensive evaluation: likelihood difference is "
            << " TODO "
            // <<  std::setprecision(8) <<   fabs (verifiedLnl - toVerify )
            << " (with toVerify= " << toVerify << ", verified="
            << verifiedLnl << ")\n"
        ...
        tout  << "after full traversal on orig="
              << traln.getTrHandle().likelihood << std::endl;
        assert(0);
    }
    ...
    // std::cout << SyncOut() << "VERIFIED " << MAX_SCI_PRECISION <<
    // verifiedLnl << SOME_FIXED_PRECISION << std::endl;
...
#ifdef DEBUG_LNL_VERIFY
void                    LikelihoodEvaluator::setDebugTraln(
...
void                    LikelihoodEvaluator::invalidateArray(
    const TreeAln&traln,
    nat           nodeId)
{
    // tout << "INVAL "<< nodeId << std::endl;
    for (nat i = 0; i < traln.getNumberOfPartitions(); ++i)
        _arrayOrientation.setOrientation(i, ArrayOrientation::nodeNum2ArrayNum(
                                             nodeId, traln.getNumberOfTaxa()),
                                         INVALID);
}


void                    LikelihoodEvaluator::debugPrintToComputeHelper(
    const TreeAln&    traln,
    const BranchPlain&root)
{
    ...
    {
        tout << root.getPrimNode() <<  "=(" << desc.first.getSecNode()
             <<  "+"  << desc.second.getSecNode()  << "),";
...
void                    LikelihoodEvaluator::debugPrintToCompute(
    const TreeAln&    traln,
    const BranchPlain&root)
{
    tout << "to compute: 0=(" <<   root.getPrimNode() << "+"
         << root.getSecNode() << "),";
...
void                    LikelihoodEvaluator::markPartitionDirty(
    const TreeAln&traln,
    nat           partition)
{
...
void                    LikelihoodEvaluator::accountForRejection(
    TreeAln&                traln,
    const std::vector<bool>&partitions,
    const std::vector<nat>& invalidNodes)
{
    _arrayPolicy->accountForRejection(traln, partitions, invalidNodes,
                                      _arrayOrientation, *_arrayReservoir);
}
...
void                    LikelihoodEvaluator::imprint(
    const TreeAln&traln)
{
...
bool                    LikelihoodEvaluator::subtreeValid(
    const TreeAln&                        traln,
    const BranchPlain&                    root,
 ...
 */
void                    LikelihoodEvaluator::invalidateWithBranches(
    const TreeAln&                        traln,
    const BranchPlain&                    root,
    const std::unordered_set<BranchPlain>&invalidBranches)
{
