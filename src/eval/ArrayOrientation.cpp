    ...
    : orientation(traln.getNumberOfPartitions())
{
    for (nat i = 0; i < traln.getNumberOfPartitions(); ++i)
        orientation[i] = std::vector<nat>(traln.getNumberOfInnerNodes(false),
                                          INVALID);
}


nat                     ArrayOrientation::nodeNum2ArrayNum(
    nat nodeNum,
    nat numTax)
{
...
bool                    ArrayOrientation::searchNodeInSubtree(
    const TreeAln&traln,
    nodeptr       p,
    nat           nodeId,
    nat           depth) const
 ...
 */
bool                    ArrayOrientation::isCorrectNew(
    const TreeAln& traln,
    nat            part,
    nodeptr        p) const
{
    ...
    // tout << "node=" << p->number << " is oriented " << SHOW(theOrient) <<
    // std::endl;
...
//  tout << "CORRECT!\t" << SHOW(p->back->number) << std::endl;
        ...
        )
    {
        ...
        auto topoRearrangement = (nat(p->next->back->number) != theOrient)  &&
            (nat(p->next->next->back->number) != theOrient);
...
void                            ArrayOrientation::
    setOrientationForAllPartitions(
    nat id,
    nat value)
{
...
void                            ArrayOrientation::setPartitionInvalid(
    nat part)
{
...
void                            ArrayOrientation::setInvalid(
    nat part,
    nat id)
{
...
nat                             ArrayOrientation::getOrientation(
    nat part,
    nat id) const
{
