...
double*                   ArrayReservoir::allocate(
    size_t requiredLength)
{
    ...
    bool   freeOldArrayDespiteAvailable =  _freeOlds &&   (size_t(
                                                               double(
                                                                   requiredLength)
                                                               *
                                                               thresholdForNewSEVArray)
                                                           < lengthOfFound);
    ...
    else
    {
        ...
        {
            auto maxElemIter = std::max_element(begin(_unusedArrays), end(
                                                    _unusedArrays));
        ...
        // tout << "allocating elem of length " <<  requiredLength<<  "\t" <<
        // SHOW(lengthOfFound)  << std::endl;
...
#ifdef _DEVEL
std::tuple<uint64_t,
           uint64_t>                    ArrayReservoir::
    getUsedAndUnusedBytes() const
{
    uint64_t usedBytes =  std::accumulate(
            begin(_usedArrays),
            end(_usedArrays),
            0,
            [](nat elem, const std::pair<double*, nat>& elemB)
    {
        return elem + std::get<1>(elemB);
    });

    uint64_t unusedBytes = std::accumulate(
            begin(_unusedArrays),
            end(_unusedArrays),
            0,
            [](nat elem, const std::pair<nat, double*>&elemB)
    {
        return elem  + std::get<0>(elemB);
    });
