...
void                    ArrayRestorer::restoreSomePartitions(
    TreeAln&                traln,
    ...
    ArrayReservoir&         res)
{
    for (nat model = 0; model <  traln.getNumberOfPartitions(); ++model)
    {
        ...
        auto& partition = traln.getPartition(partitionIndex).getHandle(); // TODO
                                                                          // lazy
        ...
        {
            std::copy(partitionLikelihoods.at(
                          partitionIndex).gapColumn.begin(),
...
void                    ArrayRestorer::cache(
    TreeAln&               traln,
    nat                    nodeNumber,
    nat                    partitionId,
    const ArrayOrientation&curOrient)
{
    // tout << "caching node=" << nodeNumber << ", partition=" << partitionId
    // << std::endl;
    ...
    partitionLikelihoods[partitionId].scaler[nodeNumber] =
        partition.globalScaler[nodeNumber];

    if (restoresGapVector)
    {
        auto vec  = partition.gapVector + nodeNumber
            * partition.gapVectorLength;
        auto iter = partitionLikelihoods[partitionId].gapVector.begin() + id
            * partition.gapVectorLength;
...
void                    ArrayRestorer::recycleArray(
    TreeAln&        traln,
    nat             nodeNumber,
    nat             partitionId,
    ArrayReservoir& res)
...
void                    ArrayRestorer::destroyAndForget(
    TreeAln&traln,
    nat     nodeNumber,
    nat     partitionId)
{
...
void                    ArrayRestorer::uncache(
    ...
    ArrayReservoir&  res)
{
    // tout << "uncaching node=" << nodeNumber << ", partition=" << partitionId
    // << std::endl;
...
void                    ArrayRestorer::traverseAndCache(
    TreeAln&          traln,
    nodeptr           virtualRoot,
    nat               model,
    ArrayOrientation& curOrient,
...
// #else
//   bool incorrect = not curOrient.isCorrect(model, index,
// virtualRoot->back->number);
// #endif
    ...
    if (incorrect && not partitionLikelihoods[model].isCached[index])
    {
        ...
        bool tipinner = (not traln.isTipNode(pnb))   !=  (not traln.isTipNode(
                                                              pnnb));
...
void                                       ArrayRestorer::resetRestorer(
    const TreeAln&   traln,
    ArrayOrientation&curOrient)
{
    ...
    for (nat i = 0; i < traln.getNumberOfPartitions(); ++i)
    {
        ...
        if (restoresGapVector && partition.width > 0)
        {
            ...
            std::copy(partition.gapColumn,
                      partition.gapColumn + traln.getNumberOfTaxa()
                      * partition.states * 4,
...
std::pair<double*, nat>                    ArrayRestorer::removeArray(
    TreeAln&traln,
    nat     num,
    nat     pid)
{
...
void                                       ArrayRestorer::insertArray(
    TreeAln&traln,
    nat num,
    nat pid,
