    ...
    , _taxa(fillTaxaInfo(fileNames.at(0)))
{
    ...
    if (expensiveCheck && fileNames.size() > 1)
    {
        for (auto f : fileNames)
        {
            ...
            if (not okay)
            {
                std::cout << "Error: file " << f
                          << " contains a different numbering of taxa than "
                          << fileNames[0] << ". At the moment, "
                          << PROGRAM_NAME
                          <<
                    " does not support this. Write us an e-mail, if this feature is important for you. "
                          << std::endl;
...
TreeProcessor&                              TreeProcessor::operator=(
    TreeProcessor&&rhs)
{
    if (this == &rhs)
        return *this;
...
void                                        TreeProcessor::nextTree(
    std::istream&treefile)
{
    auto paramPtr = make_unique<BranchLengthsParameter>(0, 0,
                                                        std::vector<nat>{0});
    ...
    for (auto b :branches)
    {
        _tralnPtr->clipNode(_tralnPtr->getUnhookedNode(b.getPrimNode()),
                            _tralnPtr->getUnhookedNode(b.getSecNode()));
...
std::vector<std::string>                    TreeProcessor::fillTaxaInfo(
    std::string fileName)
{
    ...
    while (not abort && getline(infile, line))
    {
        ...
        if (foundStart)
        {
            ...
            std::string num  =  cleanerString.substr(0, pos),
                        name = cleanerString.substr(pos + 1,
                                                    cleanerString.size());
...
template void                    TreeProcessor::nextTree<true>(
    std::istream&treefile);
template void                    TreeProcessor::nextTree<false>(
    std::istream&treefile);
