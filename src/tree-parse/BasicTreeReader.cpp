...
BasicTreeReader<LABEL_READER, BL_READER>::BasicTreeReader(
    nat numTax)
...
template<class LABEL_READER, class BL_READER>
inline void BasicTreeReader<LABEL_READER,
                            BL_READER                               >
    ::expectChar(
    std::istream&iss,
    int          ch)
{
    ...
    if (ch != got)
    {
        std::cerr << "expected >" << char(ch) << "<, got >"  <<  char(got)
                  << "<" << std::endl;
...
template<class LABEL_READER, class BL_READER>
std::tuple<nat, double>BasicTreeReader<LABEL_READER,
                                       BL_READER                    >
    ::parseElement(
    std::istream&iss)
{
...
template<class LABEL_READER, class BL_READER>
void BasicTreeReader<LABEL_READER,
                     BL_READER                                      >
    ::addBranch(
    nat label,
...
template<class LABEL_READER, class BL_READER>
std::vector<BranchLength>BasicTreeReader<LABEL_READER,
                                         BL_READER                    >::
    extractBranches(
    std::istream&iss)
{
