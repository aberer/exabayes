...
TreePrinter::TreePrinter(
    bool withBranchLengths_I,
    bool withInternalNodes_I,
    bool withRealNames_I)
...
std::string                    TreePrinter::printTree(
    const TreeAln&     traln,
...
void                           TreePrinter::printBranchLength(
    const TreeAln&                        traln,
    std::stringstream&                    ss,
    nodeptr                               p,
    const std::vector<AbstractParameter*>&params)
{
    ...
    if (params.size() == 1)
    {
        ...
        ss << ":" << traln.getBranch(p, param).toMeanSubstitutions(
    ...
    else
    {
        ...
        {
            // ss << (isFirst ? "" : ",")  <<
            // traln.getBranch(p,param).getInterpretedLength( param);
            ss << (isFirst ? "" : ",") << traln.getBranch(p,
                                                          param).
                toMeanSubstitutions(param->getMeanSubstitutionRate());
            isFirst = false;
        }
