...
T                                 ByteFile::readVar()
{
    ...
    {
        std::cout << "read " << bytesRead << " expected "  << sizeof(T)
                  << std::endl;
        assert(0);
    }
    ...
    T*     array)
{
    ...
    {
        std::cout << "problem reading " << length * sizeof(T)
                  << " instead got " << SHOW(bytesRead) << std::endl;
        assert(0);
    }
}
...
template<typename T>
std::vector<T>                    ByteFile::readArray(
    size_t length)
{
