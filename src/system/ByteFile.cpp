...
nat                                             ByteFile::determineOptStride(
    Communicator&comm)
{
...
template<typename T>
std::vector<std::vector<T> >                    ByteFile::
    readAndDistributeArrays(
    ParallelSetup&       pl,
    ...
    size_t               numberOfArrays)
{
    ...
    auto numProcPerChain =
        std::max((pl.getGlobalComm().size() / pl.getChainsParallel())
                 / pl.getRunsParallel(), size_t(1u));
    auto participatingProcs =
        numProcPerChain * pl.getChainsParallel()
        * pl.getRunsParallel();
    auto numIter =
        (numberOfArrays / participatingProcs)
        +   (((numberOfArrays % participatingProcs) == 0) ? 0 : 1);

    auto countsPerProc = std::vector<int>{};
    auto displPerProc = std::vector<int>{};
    std::tie(countsPerProc, displPerProc) = pAss.getCountsAndDispls(
            pl.getChainComm().size());

    auto arrsProcessed = size_t(0u);

    for (nat iter = 0; iter < numIter; ++iter)
    {
        auto arraysThisRound = std::min(size_t(participatingProcs),
                                        numberOfArrays - iter
                                        * participatingProcs);
        auto arrs = std::vector<std::vector<T> >(arraysThisRound);
        auto myArrNum = iter * participatingProcs
            + pl.getGlobalComm().getRank();

        if (nat(pl.getGlobalComm().getRank()) < participatingProcs)
        {
            if (myArrNum < numberOfArrays)
            {
                _in.seekg(std::streamoff(size_t(fileStart) +  (_numPat
                                                               * sizeof(T))
                                         * myArrNum), std::ios::beg);
                arrs[myArrNum - arrsProcessed] = readArray<T>(_numPat);
            }

            nat ctr = 0;

            for (auto j = myArrNum % numProcPerChain; j < arraysThisRound; j +=
                     numProcPerChain)
            {
                if (arrs[j].size() == 0)
        ...
        for (nat i = 0; i < arrs.size(); ++i)
        {
            if (arrs.at(i).size() > 0)
            {
                ...
                for (auto assignmentPair : ass)
                {
                    ...
                    std::copy(begin(arr) + beg, begin(arr) + beg
                              + assignment.width, begin(arrReordered)
                              + indexPerProc.at(
                                  assignment.procNum));
            ...
            auto&&res = pl.getChainComm().scatterVariableKnownLength<T>(
                    arrReordereds[i], countsPerProc, displPerProc, i
                    % int(numProcPerChain));
...
template std::vector<std::vector<int32_t> >                     ByteFile::
    readAndDistributeArrays(
    ParallelSetup&       pl,
    PartitionAssignment& pAss,
    size_t               numberOfArrays);
template std::vector<std::vector<uint16_t> >                    ByteFile::
    readAndDistributeArrays(
    ParallelSetup&       pl,
    PartitionAssignment& pAss,
    size_t               numberOfArrays);
template std::vector<std::vector<uint8_t> >                     ByteFile::
    readAndDistributeArrays(
    ParallelSetup&       pl,
    PartitionAssignment& pAss,
    size_t               numberOfArrays);
    ...
    PartitionAssignment& pAss)
{
    ...
    for (auto iter = range.first; iter != range.second; ++iter)
    {
        ...
        auto pos = alnPos + std::streamoff(lenOfPat *  (partition.getLower()
                                                        + assignment.offset));
        _in.seekg(pos);
        ...
        auto aln = aligned_malloc<uint8_t, size_t(EXA_ALIGN)>(
                assignment.width * _taxa.size());
        ...
        // TODO !!!!
        // tout << "NOT setting parsimony information read from file "  <<
        // std::endl;
#endif
...
void                    ByteFile::parseAlnsDirectRead(
    ParallelSetup&       pl,
    PartitionAssignment& pAss)
{
    seek(Position::ALIGNMENT);
    ...
    auto alnPos = _in.tellg();
    auto range  = pAss.getAssignment().equal_range(
            pl.getChainComm().getRank());
    ...
    for (auto iter = range.first; iter != range.second; ++iter)
    {
        ...
        {
            auto start = size_t(alnPos) + ctr * _numPat
                +  partition.getLower() + assignment.offset;
            _in.seekg(start);
    ...
    // tout << "[ "  << getDuration(_initTime) << "] computed parsimony" <<
    // std::endl;
    // tout << "io-time: " << ioTime << "\tmallocTime" << mallocTime <<
    // std::endl;
    // _initTime = getTimePoint();
}


void                    ByteFile::parseAlns(
    ParallelSetup&       pl,
    PartitionAssignment& pAss)
{
    seek(Position::ALIGNMENT);
    ...
    // tout << "[ "  << getDuration(_initTime) << "] distributed alignment" <<
    // std::endl;
    // _initTime = getTimePoint();
    ...
    for (auto iter = range.first; iter != range.second; ++iter)
    {
        auto assignment = iter->second;
        auto&partition  = _partitions.at(assignment.partId);
        auto aln = aligned_malloc<unsigned char, size_t(EXA_ALIGN)>(
                assignment.width * _taxa.size());
        ...
        {
            std::copy(begin(myAlnRow) + pos, begin(myAlnRow) + pos
                      + assignment.width, aln  + ctr * assignment.width);
            ++ctr;
        }

        pos += assignment.width;
        partition.setAlignment(shared_pod_ptr<unsigned char>(aln),
                               assignment.width);
    }
    ...
    PartitionAssignment&pAss)
{
    ...
    for (auto iter = range.first; iter != range.second; ++iter)
    {
        ...
        _in.seekg(wgtPos + (partition.getLower() + assignment.offset)
                  * sizeof(T));
        readArray<T>(assignment.width, arrForReading.data());

        // more efficient copy possible? probably not, since we change the size
        // of the integer
...
template void                    ByteFile::parseWeights<uint32_t>(
    ParallelSetup&      pl,
    PartitionAssignment&pAss);
template void                    ByteFile::parseWeights<uint16_t>(
    ParallelSetup&      pl,
    PartitionAssignment&pAss);
template void                    ByteFile::parseWeights<uint8_t>(
    ParallelSetup&      pl,
    PartitionAssignment&pAss);


void                             ByteFile::parseTaxa(
    int numTax)
{
    seek(Position::TAXA);
    ...
    int numPart)
{
    ...
    for (int i = 0; i < numPart; ++i)
    {
        ...
        auto  partition =  Partition(nat(_taxa.size()), str, dataType, states,
                                     maxTipStates, _saveMemory);
        ...
        // tout << "parsed partition " << i << ": " << partition.getLower() <<
        // "\t" << partition.getUpper() << "\twidth" << partition.getWidth()
        //  << std::endl;
    }
    ...
    Position pos)
{
    ...
    case Position::ALIGNMENT:   // skips partitions
    {
        ...
        auto   oneElem =
            sizeof(ps.states) + sizeof(ps.maxTipStates) + sizeof(ps.lower)
            + sizeof(ps.upper)
            + sizeof(ps.dataType) + sizeof(ps.protModels)
            + sizeof(ps.protUseEmpiricalFreqs) + sizeof(ps.nonGTR);
        toSkipHere += _partitions.size() * (oneElem + sizeof(int));

        for (auto&p : _partitions)
            toSkipHere += (p.getName().size() + 1)  * sizeof(char); // TODO
...
void                                      ByteFile::checkMagicNumber()
{
    ...
    if (std::string(tmp).compare("BINARY") != 0)
    {
        tout
            <<
            "expected different file start (maybe reparse the binary file). Got >"
            << tmp << "<" << std::endl;
        assert(0);
    }
}
