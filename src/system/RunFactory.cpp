    ...
    const TreeAln&                                   traln) const
{
    ...
    for (auto c : allCats)
    {
        cat2partsUsed[c] = std::vector<bool>(traln.getNumberOfPartitions(),
                                             false);
        cat2idOfItsKind[c] = -1;
    }
    ...
    for (const auto&p : params)
    {
        ...
        if (p->getCategory() == Category::SUBSTITUTION_RATES
            &&  traln.getPartition(p->getPartitions()[0]).getDataType() ==
            PLL_AA_DATA)
        {
            for (auto part : p->getPartitions())
            {
                if (not cat2partsUsed.at(relCat).at(part))
                {
                    ...
                    paramsToAdd.push_back(
                        CategoryFuns::getParameterFromCategory(relCat,
                                                               highestParamId,
                                                               cat2idOfItsKind[
                                                                   relCat], {part},
                                                               traln.
    ...
    for (auto cat : CategoryFuns::getAllCategories())
    {
        ...
        case Category::FREQUENCIES:
            ...
            for (nat i = 0; i < traln.getNumberOfPartitions(); ++i)
            {
                ...
                if (not cat2partsUsed.at(cat).at(i) // not already there
                    && dataType != PLL_AA_DATA) // we use an AA_MODEL as
        ...
        case Category::SUBSTITUTION_RATES:
        {
            for (nat i = 0; i < traln.getNumberOfPartitions(); ++i)
            {
                ...
                if (not cat2partsUsed.at(cat).at(i) // not already there
                    ...
                    && dataType != PLL_BINARY_DATA) // we use an AA_MODEL as
                                                    // default for proteins
                    partsUnused.push_back(i);
            }
        }
        ...
        if (CategoryFuns::inUniqueByDefault(cat))
        {
            ...
            for (int i = 0; i < numNeeded; ++i)
            {
                ++highestParamId;
                ++cat2idOfItsKind[cat];
                params.push_back(CategoryFuns::getParameterFromCategory(cat,
                                                                        highestParamId,
                                                                        cat2idOfItsKind
                                                                        [cat],
                                                                        partsUnused,
...
void                    RunFactory::addStandardPrior(
    AbstractParameter* var,
    const TreeAln&     traln)
{
    switch (var->getCategory())         // TODO such switches should be part of
                                        // an object
    {
    case Category::TOPOLOGY:
        var->setPrior(make_unique<UniformPrior>(0, 0)); // TODO : proper
                                                        // topology prior?
        break;
    ...
    case Category::SUBSTITUTION_RATES:
    {
        auto& partition = traln.getPartition(var->getPartitions()[0]);
        ;
        var->setPrior(make_unique<DirichletPrior>(std::vector<double>(
                                                      RateHelper::
                                                          numStateToNumInTriangleMatrix(
                                                          partition.getStates()),
                                                      1.)));
    }
    break;
...
void                    RunFactory::addPriorsToParameters(
    const TreeAln&   traln,
    ...
    ParameterList&   variables)
{
    ...
    for (auto&v : variables)
    {
        ...
        for (auto iter = priors.find(cat); iter != priors.end(); ++iter)
        {
            ...
            foundPrior = std::any_of(begin(partitionIds), end(partitionIds),
                                     [&](nat tmp)
            {
                return partitionsOfPrior.find(tmp) != partitionsOfPrior.end();
            });
            ...
            if (foundPrior)
            {
                ...
                {
                    tout  << "You forced prior " << &prior
                          << " to be applied to parameter  "  << v
                          << ". This is not possible. "  << std::endl;
                    exitFunction(-1, true);
                }
...
void                    RunFactory::addSecondaryParameters(
    ...
    ->std::tuple<std::vector<AbstractProposal::UPtr>, std::vector<ProposalSet> >
{
    ...
    for (auto&v : params)
    {
        ...
        {
            // bool isProtPartition =
            // traln.getPartition(v->getPartitions().at(0)).dataType ==
            // AA_DATA;
            ...
            {
                if (not (elem->isForProteinOnly() && isDNAPartition)) // TODO
                                                                      // more
                                                                      // generic!
    ...
    if (componentWiseMH)
    {
        ...
        for (auto&elem : cat2param)
        {
            ...
            {
                // bool isProtPartition =
                // traln.getPartition(std::get<1>(elem).at(0)->getPartitions().at(0)).dataType
                // == AA_DATA;
                ...
                {
                    if (not (elem2->isForProteinOnly() &&  isDNAPartition)) // TODO
                                                                            // more
                                                                            // generic!
