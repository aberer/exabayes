...
{
    return CLOCK::duration_cast<CLOCK::duration<double> >(
        CLOCK::system_clock::now() - tp).count();
}


auto                      TimeTracker::getTimePoint()
...
auto                      TimeTracker::formatForWatch(
    double time)
    ->std::array<double, 3>
{
