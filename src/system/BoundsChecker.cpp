...
bool                    BoundsChecker::checkFrequencies(
...
bool                    BoundsChecker::checkBranch(
...
bool                    BoundsChecker::checkRevmat(
    const std::vector<double>&rates)
{
    ...
    for (auto&r : rates)
    {
        auto prob =  (r <= BoundsChecker::rateMax || fabs(
                          BoundsChecker::rateMax - r) < dirtyThresh)
            &&  (BoundsChecker::rateMin <= r  || fabs(BoundsChecker::rateMin
                                                      - r) < dirtyThresh);

        if (not prob)
            tout << MAX_SCI_PRECISION << "attention: problem  with " << r
                 << std::endl;
    ...
    {
        tout << "error: last state was " << MAX_SCI_PRECISION
             << rates.back() << std::endl;
        assert(0);
    }
...
bool                    BoundsChecker::checkAlpha(
    double alpha)
{
    return BoundsChecker::alphaMin <= alpha    && alpha <=
           BoundsChecker::alphaMax;
}


void                    BoundsChecker::correctAlpha(
    double&alpha)
{
...
void                    BoundsChecker::correctBranch(
...
void                    BoundsChecker::correctRevMat(
...
void                    BoundsChecker::correctFrequencies(
    std::vector<double>&frequencies)
{
    ...
    sum = 1. - (static_cast<double>(frequencies.size()) - numberOkay)
        * BoundsChecker::freqMin;

    // renormalize again
    for (auto&f : frequencies)
        if (std::fabs(f -  BoundsChecker::freqMin) >
