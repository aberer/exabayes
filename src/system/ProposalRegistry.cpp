...
auto                    ProposalRegistry::getSingleParameterProposals(
    Category                  cat,
    const BlockProposalConfig&config,
    const TreeAln&            traln,
    ParallelSetup&            pl,
    ParameterList&            params) const
    ->std::vector<AbstractProposal::UPtr>
{
    auto   result = std::vector<AbstractProposal::UPtr>{};

    auto   numNodeAges = traln.getNumberOfInnerNodes(true);

    auto&& proposals =
        ProposalTypeFunc::getSingleParameterProposalsForCategory(cat);
    ...
    for (auto& p : proposals)
    {
        ...
        auto&& proposal = AbstractProposal::UPtr();

        switch (p)
        {
        case ProposalType::ST_NNI:
        {
            auto tmp = make_unique<GenericTopoProposal>(
                    make_unique<StatNniProposer>(), "stNNI", 6.,
                    config.getMoveOptMode());
        ...
        case ProposalType::BRANCH_LENGTHS_MULTIPLIER:
            proposal = make_unique<BranchLengthMultiplier>(
                    initBranchLengthMultiplier);
            break;
        case ProposalType::NODE_SLIDER:
            proposal = make_unique<NodeSlider>(initNodeSliderMultiplier);
            break;
        case ProposalType::TL_MULT:
            proposal = make_unique<TreeLengthMultiplier>(
                    ProposalRegistry::initTreeLengthMultiplier);
            break;
        case ProposalType::E_TBR:
        {
            auto&& tmp = make_unique<GenericTopoProposal>(
                    make_unique<ExtendedTbrProposer>(config.getEtbrStopProb()),
                    "eTBR", 5., config.getMoveOptMode());
        ...
        {
            auto tmp = make_unique<GenericTopoProposal>(
                    make_unique<ExtendedSprProposer>(config.getEsprStopProp()),
                    "eSPR", 6., config.getMoveOptMode());
        ...
        case ProposalType::LIKE_SPR:
        {
            ...
            if (rad == -1)
                rad = int(std::ceil(std::log(traln.getNumberOfTaxa())
                                    / std::log(2)));

            proposal = make_unique<GenericTopoProposal>(
                    make_unique<LikehoodSprProposer>(rad,
                                                     config.getLikeSprWarp(),
                                                     config.getMoveOptMode()),
                    "likeSPR", 2.,
                    config.getMoveOptMode());
        }
        ...
        case ProposalType::PARSIMONY_SPR:
        {
            ...
            auto&& tmp  = make_unique<GenericTopoProposal>(
                    make_unique<ParsSprProposer>(config.getParsimonyWarp(),
                                                 radius,
                                                 pl.getChainComm()), "parsSpr",
                    6., config.getMoveOptMode());
        ...
        case ProposalType::REVMAT_SLIDER:
            proposal = make_unique<ParameterProposal>(
                    Category::SUBSTITUTION_RATES, "revMatSlider", true,
                    std::make_unique<
                        SlidingProposer>(BoundsChecker::rateMin,
                                         BoundsChecker::rateMax,
                                         true),
                    initRateSlidingWindow, 1,
                    1e-5, 1);
            break;
        case ProposalType::FREQUENCY_SLIDER:
            proposal = std::make_unique<ParameterProposal>(
                    Category::FREQUENCIES,
                    "freqSlider", true,
                    std::make_unique<SlidingProposer>(
                        BoundsChecker::freqMin,
        ...
        case ProposalType::RATE_HET_MULTI:
            proposal = make_unique<ParameterProposal>(
                    Category::RATE_HETEROGENEITY, "rateHetMulti", false,
                    std::make_unique<MultiplierProposer>(
                        BoundsChecker::alphaMin, BoundsChecker::alphaMax),
                    initGammaMultiplier, 1,
                    1e-4, 1e2);
            break;
        case ProposalType::RATE_HET_SLIDER:
            proposal = make_unique<ParameterProposal>(
                    Category::RATE_HETEROGENEITY, "rateHetSlider", false,
                    std::make_unique<SlidingProposer>(BoundsChecker::alphaMin,
                                                      BoundsChecker::alphaMax,
                                                      false),
        ...
        case ProposalType::FREQUENCY_DIRICHLET:
            proposal = make_unique<ParameterProposal>(
                    Category::FREQUENCIES,
                    "freqDirich", true,
                    std::make_unique<DirichletProposer>(
                        BoundsChecker::freqMin,
                        std::numeric_limits
                        <double>::max(),
                        false),
        ...
        {
            proposal = make_unique<ParameterProposal>(
                    Category::SUBSTITUTION_RATES, "revMatDirichRate", true,
                    std::make_unique<
                        RateDirichletProposer>(BoundsChecker::rateMin,
                                               BoundsChecker::rateMax),
        ...
        {
            proposal = std::make_unique<ParameterProposal>(
                    Category::SUBSTITUTION_RATES, "revMatSliderRate", true,
                    std::make_unique<
                        RateSlidingProposer>(BoundsChecker::rateMin,
                                             BoundsChecker::rateMax),
        ...
        case ProposalType::BL_DIST_WEIBULL:
            proposal =
                make_unique<DistributionBranchLength<WeibullProposer> >();
            break;
        default:
        {
            cerr << "you did not implement case " << int(p)
                 << " in ProposalRegistry.cpp" << endl;
            assert(0);
        }
        }
