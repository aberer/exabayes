...
static inline nat                    openNewPartition(
    std::vector<PartInfo>::iterator&iter,
    std::vector<PartInfo>::iterator end)
{
    auto result = 0;
...
void                                 PartitionAssignment::improvedAssign(
    std::vector<PartInfo>partitions)
{
    std::sort(begin(partitions), end(partitions), [](const PartInfo& a,  const
                                                     PartInfo&b)
    {
        return a.num < b.num;
    });
    ...
    while (iterate)
    {
        ...
        {
            if (partIter != end(partitions) && sizeAssigned[proc]
                + partIter->num <= cap)
            {
    ...
    int  highProc =  procsHigh.empty() ? popAndYield(procsLow) : popAndYield(
            procsHigh);
    int  lowProc  =  procsLow.empty() ? popAndYield(procsHigh) : popAndYield(
            procsLow);

    while (not (procsLow.empty() &&  procsHigh.empty() && highProc == -1 &&
                lowProc == -1))
    {
        if (highProc != -1  && sizeAssigned[highProc] + toAdd >= cap)
        {
            ...
            _assignToProcPartially(*partIter, highProc, numAssigned,
                                   sizeAssigned, offset, toTransfer);
            toAdd -= toTransfer;
            ...
            highProc = procsHigh.empty() ? popAndYield(procsLow) : popAndYield(
                    procsHigh);
        }
        else if (lowProc != -1)
        {
            ...
            {
                auto offset = partIter->num - toAdd;
                _assignToProcPartially(*partIter, lowProc, numAssigned,
                                       sizeAssigned, offset, toAdd);
                ...
                lowProc = procsLow.empty() ? popAndYield(procsHigh) :
                    popAndYield(procsLow);
            }
            else
            {
                ...
                lowProc = procsHigh.empty() ? popAndYield(procsLow) :
                    popAndYield(procsHigh);
            }
        }
    }
...
void                    PartitionAssignment::_assignToProcFull(
    PartInfo         p,
    nat              proc,
    std::vector<nat>&numAssigned,
    std::vector<nat>&sizeAssigned)
{
    auto assnmnt = Assignment{
        p.id, proc, 0, p.num, 0
    };
...
void                    PartitionAssignment::_assignToProcPartially(
    PartInfo         p,
    nat              proc,
    std::vector<nat>&numAssigned,
    std::vector<nat>&sizeAssigned,
...
void                    PartitionAssignment::assignForType(
    std::vector<PartInfo>curParts)
{
    ...
    std::sort(begin(curParts), end(curParts), [](const PartInfo&a, const
                                                 PartInfo& b)
    {
        return a.num < b.num;
    });
    ...
    auto comparator = [](const std::pair<int, int>&a, const std::pair<int,
                                                                      int>& b)
        {
    ...
    auto numPartAndSites2Proc = std::multimap<std::pair<int, int>, int,
                                              decltype(comparator)>(
            comparator);
    ...
    for (auto part : curParts)
    {
        ...
        while (toAssign > 0)
        {
            ...
            // create assignment
            _proc2assignment.insert(std::make_pair(proc, Assignment{part.id,
                                                                    nat(proc),
                                                                    nat(
                                                                        offset),
                                                                    nat(
            ...
            if (sitesLeft[proc] > 0)
                numPartAndSites2Proc.insert(std::make_pair(std::make_pair(
                                                               numParts,
                                                               numSites),
                                                           proc));
        }
...
void                    PartitionAssignment::assign(
    const std::vector<Partition>& partitions)
{
    ...
    for (auto&p : partitions)
    {
        ...
        typeToPart.insert(std::make_pair(type, PartInfo{ctr, nat(p.getUpper()
                                                                 - p.getLower()),
                                                        0}));
        ++ctr;
    }
    ...
    std::unique_copy(begin(typeToPart),
                     ...
                     [](const std::pair<nat, PartInfo>&entry1, const
                        std::pair<nat, PartInfo>&entry2)
    {
...
void                    PartitionAssignment::assignOld(
    const std::vector<Partition>& partitions)
{
    ...
    std::sort(begin(ps), end(ps), [](const PartInfo& a, const PartInfo&b)
    {
        return a.num * a.compStates > b.num * b.compStates;
    });
    ...
    for (auto&p : ps)
    {
        ...
        {
            nat elemsToTransfer = nat(ceil(double(std::min(partitionElemsLeft
                                                           * p.compStates,
                                                           loadPerProc
                                                           - curLoad))
                                           / double(p.compStates)));
            ...
            _proc2assignment.insert(std::make_pair(curProc, Assignment{p.id,
                                                                       curProc,
                                                                       offset,
                                                                       elemsToTransfer,
                                                                       p.
    ...
    nat proc) const
{
    ...
    for (auto iter = std::get<0>(iterPair); iter != std::get<1>(iterPair);
         ++iter)
...
std::pair<std::vector<int>,
          std::vector<int> >
                    PartitionAssignment
    ::getCountsAndDispls(
    size_t numInComm) const
{
    auto countsPerProc = std::vector<int>{};

    for (nat i = 0; i <  numInComm; ++i)
        countsPerProc.push_back(int(getTotalWidthPerProc(i))); // TODO
    ...
    for (nat i = 1; i  < numInComm; ++i)
        displPerProc.push_back(int(displPerProc[i - 1] + countsPerProc[i
                                                                       - 1]));
