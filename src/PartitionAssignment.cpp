...
void PartitionAssignment::improvedAssign( std::vector<PartInfo> partitions )
{
  ...
  auto remainder = cap * _numProc -  __totalSites ; 
  tout << SHOW(remainder ) << std::endl;
  auto numFull = 0u; 
...
#if 0 
  // omitting for this version 
  // phase 3: top up 
  for(auto proc = low ; proc < _numProc; ++proc )
    {
      if( sizeAssigned[proc] + partIter->num <= cap )
  	{
	  // should never happen? 
	  // assert(0); 
...
void PartitionAssignment::assignForType(  std::vector<PartInfo> curParts )
{
  ...
  nat remainder = totalSites - (sitesPerProc * _numProc); 
