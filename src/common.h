...
#define FLOAT_IS_INITIALIZED(x) (std::fabs(x) > \
                                 std::numeric_limits<double>::epsilon())


#define USE_SSE (defined(HAVE_SSE3) &&  !defined(MANUAL_SSE_OVERRIDE))
#define USE_AVX (defined(HAVE_AVX) && !defined(MANUAL_AVX_OVERRIDE) && \
                 !defined(MANUAL_SSE_OVERRIDE))
...
#define MAX_SCI_PRECISION  std::scientific << std::setprecision( \
...
#define TARGET_RATIO 0.25    ///  the golden acceptance ratio, we want to
                             // achieve
