...
void                                        TreeAln::copyTopologyAndBl(
    const TreeAln&rhs)
{
    assert(&rhs != this);
    unlinkTree();
...
nat                                         TreeAln::addNodeToPartialTree(
    nat id,
    nat curRoot,
    nat outerCtr)
{
    ...
    ArrayReservoir&arrayReservoir)
{
    ...
    {
        auto& partition = getPartition(i).getHandle(); // TODO: no partition
                                                       // handle
...
BranchPlain                                 TreeAln::getThirdBranch(
...
std::vector<BranchPlain>                    TreeAln::getBranchesFromNode(
    nat aNode) const
{
    // tout << "TODO: make this method work for outer nodes as well"<<
    // std::endl;
    ...
    if (aNode <= getNumberOfTaxa())
    {
        ...
        return {
                   BranchPlain(aNode, p->back->number)
        };
    }
    else
    {
...
// prunedBranch.getThirdBranch(*this,subtree);
    ...
    clipNode(findNodePtr(desc.first.getInverted()), findNodePtr(
                 desc.second.getInverted()));
    ...
    tout << "PRUNE: " << SHOW(subtree) << SHOW(result) << SHOW(
        newBranch.toPlain()) << std::endl;
#endif
    ...
    const std::vector<AbstractParameter*>&params)
{
    ...
    auto oB = BranchPlain(subtree.getPrimNode(), insertBranch.getOtherNode(
                              branchToCreate.getOtherNode(
                                  subtree.getPrimNode())));
    auto otherBranchToCreate =  BranchLengths(oB, getBranch(insertBranch,
                                                            params).getLengths());
...
#ifdef PRINT_LIKESPR_INFO
    tout << "INSERT: " << SHOW(subtree) << SHOW(insertBranch) << SHOW(
        branchToCreate) << std::endl;
#endif
}


void                                                   TreeAln::clipNodeDefault(
    nodeptr p,
    nodeptr q)
{
    clipNode(p, q);
...
nodeptr                                                TreeAln::getUnhookedNode(
    int number)
{
    ...
    std::cerr << "Error: did not find unlinked node for " << number
              << std::endl;
    assert(0);
    return NULL;
}
...
size_t                                                 TreeAln::
    getNumberOfPartitions() const
{
    return _partitions.size();
}


const Partition&                                       TreeAln::getPartition(
    nat model)  const
{
    return _partitions.at(model);
}

Partition&                                             TreeAln::getPartition(
    nat model)
{
    return _partitions.at(model);
}
...
std::vector<bool>                                      TreeAln::getExecModel()
const
{
...
std::vector<log_double>                                TreeAln::
    getPartitionLnls() const
{
    ...
    bool                      isRaw)
{
    ...
    if (not valuesOkay)
    {
        tout << "Problem with substitution parameter: "
             << MAX_SCI_PRECISION << values << std::endl;
        assert(valuesOkay);
    }

    auto& partition = getPartition(model);

    assert(partition.getDataType() != PLL_AA_DATA ||
           partition.getProtModels() == PLL_GTR);

    nat   num = RateHelper::numStateToNumInTriangleMatrix(
            partition.getStates());
    ...
    const std::vector<AbstractParameter*>params)
{
    ...
    {
        auto length = branch.getLengths().at(
                param->getIdOfMyKind()).getValue();
 ...
 *
 */
void                                                   TreeAln::discretizeGamma(
    nat model)
{
    auto& partition =  getPartition(model).getHandle();
    pllMakeGammaCats(partition.alpha, partition.gammaRates,
                     Partition::maxCategories, _tr.useMedian);
}
...
std::vector<double>                                    TreeAln::getRevMat(
    nat  model,
    bool isRaw) const
{
    auto& partition = getPartition(model);
    ...
    nat                       model)
{
    ...
    auto& partition = getPartition(model);
    assert(partition.getDataType() != PLL_AA_DATA ||
           partition.getProtFreqs() == PLL_TRUE);
...
std::vector<double>                                    TreeAln::getFrequencies(
    nat model) const
{
    auto& partition = getPartition(model);
    assert(partition.getDataType() != PLL_AA_DATA ||
...
nodeptr                                                TreeAln::getNode(
    nat elem) const
{
    if (not (elem != 0 && elem < getNumberOfNodes() + 2))
    {
        std::cout << "bug: attempted to get node " << elem << std::endl;
        assert(elem != 0 && elem <= getNumberOfNodes() + 1);
    }
...
std::pair<BranchPlain, BranchPlain>                    TreeAln::getDescendents(
    const BranchPlain&b) const
{
    ...
    auto pn  = p->next,
         pnn = p->next->next;
    return std::make_pair(BranchPlain(pn->number, pn->back->number),
                          BranchPlain(pnn->number, pnn->back->number));
}


nat                                                    TreeAln::getDepth(
    const BranchPlain&b) const
{
    if (isTipBranch(b))
        return 1;
...
std::vector<nat>                                       TreeAln::
    getLongestPathBelowBranch(
    const BranchPlain&b) const
{
    if (isTipBranch(b))
        return std::vector<nat>{
                   b.getSecNode()
        };
    else
    {
...
BranchPlain                                            TreeAln::getAnyBranch()
const
{
    // assert(0);
...
BranchLength                                           TreeAln::getBranch(
    const nodeptr&          p,
...
BranchLengths                                          TreeAln::getBranch(
    const nodeptr&                        p,
...
std::vector<BranchPlain>                               TreeAln::extractBranches()
const
{
...
std::vector<BranchLength>                              TreeAln::extractBranches(
    const AbstractParameter* param) const
{
    ...
    for (int i = getNumberOfTaxa() + 1; i < int(getNumberOfNodes() + 1 ); ++i)
    {
        ...
        if (aNode->back->number < i)
            result.emplace_back(getBranch(BranchPlain(i, aNode->back->number),
                                          param));

        if (aNode->next->back->number < i)
            result.emplace_back(getBranch(BranchPlain(i,
                                                      aNode->next->back->number),
                                          param));

        if (aNode->next->next->back->number < i)
            result.emplace_back(getBranch(BranchPlain(i,
                                                      aNode->next->next->back->
                                                          number), param));
    }
...
std::vector<BranchLengths>                             TreeAln::extractBranches(
...
std::vector<BranchPlain>                               TreeAln::
...
ProtModel                                              TreeAln::getProteinModel(
    int part) const
{
...
partitionList&                                         TreeAln::
    getPartitionsHandle()
{
    return _partitionListResource;
}

const partitionList&                                   TreeAln::
    getPartitionsHandle() const
{
    return _partitionListResource;
}
    ...
    bool                         initial)
{
    ...
    _partitionListResource.numberOfPartitions =
        static_cast<int>(_partitions.size());
...
bool                                                   TreeAln::isTipBranch(
    const BranchPlain&branch) const
{
    return branch.getPrimNode() <= getNumberOfTaxa() || branch.getSecNode() <=
           getNumberOfTaxa();
}
    ...
    const BranchPlain&branch) const
{
    ...
    return std::any_of(begin(branches), end(branches),
                       [&](const BranchPlain&b)
    {
        return b.getSecNode() == branch.getSecNode();
    });
}
