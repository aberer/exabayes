    ...
    if (getDataType() == PLL_AA_DATA)
    {
        ...
        * pl = getPartitionLengths(&_partition);
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            _partition.tipVector_LG4[i]   =  _tipVector.data()
                + (pl->tipVectorLength * i);
            _partition.substRates_LG4[i]  = _substRates.data()
                + (pl->substRatesLength * i);
            _partition.frequencies_LG4[i] = _frequencies.data()
                +  (pl->frequenciesLength * i);
        }
    }
}
    ...
    const std::vector<bool>&informative)
{
    ...
    if (compressedEntries % INTS_PER_VECTOR != 0)
        compressedEntriesPadded = compressedEntries + (INTS_PER_VECTOR
                                                       - (compressedEntries
                                                          % INTS_PER_VECTOR));
    else
    ...
    auto ptr = aligned_malloc<nat, size_t(EXA_ALIGN)>(compressedEntriesPadded
                                                      * states * totalNodes);
    ...
    for (nat taxIter = 1; taxIter <= _numTax; taxIter++)
    {
        ...
        for (int k = 0; k < states; k++)
            compressedTips[k] = _partition.parsVect
                + ((compressedEntriesPadded * states * taxIter)
...
std::vector<bool>                    Partition::determineUninformativeSites()
const
{
...
void                                 Partition::prepareParsimony()
{
    ...
    else
    {
        ...
        // tout << "determine uninfo: "<< getDuration(tp)  << " sec"<<
        // std::endl;
...
std::ostream&                        Partition::printAlignment(
    std::ostream&out)
{
    for (nat i = 1; i < _numTax + 1; ++i)
    {
    ...
    const Partition& rhs)
{
    out << SHOW(rhs._saveMemory)
        ...
        << SHOW(std::vector<double>(begin(rhs._tipVector), end(
                                    rhs._tipVector)))
        << SHOW(rhs._EIGN)
        << SHOW(rhs._EI)
        ...
        << SHOW(rhs._gapVector)
        << SHOW(std::vector<double>(begin(rhs._gapColumn), end(
                                    rhs._gapColumn)));
    return out;
}
