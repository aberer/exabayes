    ...
    const std::string& modelString)
{
    ...
    std::transform(modelString.begin(), modelString.end(), std::back_inserter(
                       lowInputModel),  lower);
    ...
    for (auto model : getAllModels())
    {
        ...
        std::transform(modelStringHere.begin(), modelStringHere.end(),
                       std::back_inserter(lowModel), lower);
    ...
    ProtModel mod)
{
    ...
    default:
    {
        std::cerr
                <<
                "error: encountered unknown protein model with internal value "
                << int(mod) << std::endl;
        assert(0);
    }
    }
