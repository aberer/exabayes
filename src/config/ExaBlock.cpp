    ...
    NxsString&string) const
{
    ...
    else
    {
        cerr
            <<
            "ERROR while parsing boolean value: expected either \"true\" or \"false\""
            << endl;
        assert(0);
        return false;
    }
...
double                    ExaBlock::parseScientificDouble(
    NxsToken& token)  const
{
    ...
    if (sawE &&  (token.GetToken().EqualsCaseInsensitive("+") ||
                  token.GetToken().EqualsCaseInsensitive("-")))
    {
        str += token.GetToken();
        token.GetNextToken();
    }
