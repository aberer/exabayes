...
static int                         myConvertToInt(
    NxsString&elem)
{
    return int(elem.ConvertToDouble());
}

static uint64_t                    myConvertToLongInt(
    NxsString& elem)
{
        ...
        if (res != NxsBlock::NxsCommandResult(HANDLED_COMMAND))
        {
            ...
            else if (key.EqualsCaseInsensitive("convergencecriterion"))
            {
                ...
                else
                {
                    std::cerr
                        <<
                    "Error: valid values for config entry >convergenceCriterion< are 'mean', 'max' or 'none'."
                        << std::endl;
                    exitFunction(-1, true);
                }
            }
            else
                cerr << "WARNING: ignoring unknown value >"  << key
                     << "< and >" << value <<  "<" << endl;
        }
    }
}
    ...
    std::string name)
{
    ...
    {
        std::cerr << "Error: >" << name << "< must be in the interval "
                  << lowBracket  << "0,1" << upperBracket << std::endl;
        exitFunction(-1, true);
    }
}
...
void                           BlockRunParameters::verify()   const
{
    ...
    if (diagFreq <= nat(samplingFreq))
    {
        std::cerr
            <<
        "diagFreq < samplingFreq. Please choose the sampling frequency smaller than the diagnosis frequency.  "
            << std::endl;
        exitFunction(-1, true);
    }
}
