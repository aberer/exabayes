    ...
    char**argv)
{
    ...
    while ((c = getopt(argc, argv, "c:df:vhn:w:s:t:R:r:M:C:m:Sq:zxT:")) != EOF)
    {
        try
        {
            ...
            default:
            {
                std::cerr << "Encountered unknown command line option -"
                          <<  char(c)
                          <<
                    "\n\nFor an overview of program options, please use -h"
                          << std::endl;
        ...
        {
            std::cerr << "Invalid argument >" << optarg << "< to option >"
                      << reinterpret_cast<char*>(&c) << "<" << std::endl;
            exitFunction(-1, true);
        }
    }
        ...
        && (_totalThreads % (runNumParallel * chainNumParallel)) != 0)
    {
        std::cerr
            <<
            "Error: for the threaded version it is currently necessary that the number of threads is a multiple of the product of the number of chains and runs that is executed in parallel."
            << std::endl;
        exitFunction(-1, true);
    }
    ...
    if (seed.v[0] != 0 && checkpointId.compare("") != 0)
    {
        std::cout << std::endl
                  <<
            "You provided a seed and run-id for a restart from a checkpoint.\n"
                  << "Please be aware that the seed will be ignored."
                  << std::endl;
    }

    if (checkpointId.compare("") == 0 && seed.v[0] == 0)
    {
        std::cerr << "please specify a seed via -s seed (must NOT be 0)"
                  << std::endl;
        exitFunction(-1, true);
    }
    ...
    {
        std::cout << std::endl
                  << "Could not find the provided working directory >"
                  << workDir
                  << "<" << std::endl;
        exitFunction(-1, true);
    }
    ...
    {
        std::cerr << "please specify an alignment file via -f file"
                  <<  std::endl
                  <<
            "You have to transform your NEWICK-style alignment into a binary file using the appropriate parser (see manual)."
                  << std::endl;
    ...
    if (alnFileIsBinary())
    {
        if (singleModel.compare("") != 0 || modelFile.compare("") != 0)
        {
            std::cout
                <<
                "Found binary alignment file. Additionally, you provided a model file\n"
    ...
    {
        if (singleModel.compare("") == 0 && (modelFile.compare("") == 0 ||
                                             not std::ifstream(modelFile)))
        {
            std::cout
                <<
                "Found a phylip-style alignment file. However, you did not provide a\n"
                <<
    ...
    {
        std::cout << "Could not find tree file passed via -t >"  << treeFile
                  << "<" << std::endl;
        exitFunction(-1, true);
    }
}
...
void                    CommandLine::printVersion(
    std::ostream& out)
{
    out  << "This is " << PROGRAM_NAME << ", version " << PACKAGE_VERSION
         << std::endl
         <<
        "================================================================\n\n"
         << "For bugs reports and feature inquiries, please send an email to "
    ...
    std::ostream& out)
{
    ...
    out << std::endl << "Usage: " << (isYggdrasil ? "./yggdrasil" :
                                      "./exabayes")
        <<
        " -f alnFile [ -q modelFile ] [ -m model ] [ -s seed | -r id ]  -n id [options..] "
        << std::endl;
    ...
    out << "\n"
           ...
           "    -z               quiet mode. Substantially reduces the information printed by "
        << PROGRAM_NAME << ".\n"

                       "                      This option will save you some idle time, when you run "
        << PROGRAM_NAME << " with a\n"
                       "                      lot of processes.\n\n"

                       "    -d               execute a dry-run. Procesess the input, but does not execute any sampling.\n\n"
                       "    -c confFile      a file configuring your "
        << PROGRAM_NAME
        << " run. For a template see the 'examples' folder.\n\n"
...
void                         CommandLine::assertFileExists(
    std::string filename)
{
    ...
    if (not in)
    {
        std::cerr << "could not file file " << filename << ". Aborting."
                  << std::endl;
        exitFunction(-1, true);
    }
}
