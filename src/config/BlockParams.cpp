...
extern void                    genericExit(
    int code);


void                           BlockParams::partitionError(
    nat    partition,
    size_t totalPart) const
{
    std::cerr
        <<
        "In the parameter block of the configuration file you specified partition "
        << partition << ". However, there are only " << totalPart
        << " partitions in total in your alignment." << std::endl;

    std::cerr
        <<
        "NOTICE that the first partition has id 0 and the last of n partitions has the id (n-1) . "
        << std::endl;
    exitFunction(-1, true);
}


void                           BlockParams::parseScheme(
    NxsToken& token,
    Category  cat,
    nat&      idCtr)
{
    ...
    while (not token.GetToken().EqualsCaseInsensitive(")"))
    {
                       ...
                       ",")))
        {
            ...
            nat  start = (isLinkedRange || isUnlinkedRange)  ?
                schemePart.back() + 1  : part;
            nat  end = part + 1;
            ...
            for (nat i = start;  i < end; ++i)
            {
                ...
                {
                    tout << "error: partition " << i
                         <<
                        " occurring twice in the same scheme. Check your parameter-block!"
                         << std::endl;
                    exitFunction(-1, true);
                }
    ...
    for (auto schemePart : scheme)
    {
        ...
        parameters.push_back(
            CategoryFuns::getParameterFromCategory(
                cat,
                idCtr,
                getNumSeen(cat),
                schemePart,
                tralnPtr->getNumberOfTaxa()));
    ...
    NxsToken&token)
{
    ...
    while (true)
    {
        ...
        if (res != NxsBlock::NxsCommandResult(HANDLED_COMMAND))
        {
            ...
            {
                cerr
                    << "parsing error: found a linking scheme for category  "
                    <<  CategoryFuns::getShortName(cat) << " twice. Aborting.";
                exitFunction(-1, true);
            }
