    ...
    while (true)
    {
        ...
        if (res != NxsBlock::NxsCommandResult(HANDLED_COMMAND))
        {
            ...
            if (ProposalTypeFunc::isValidName(key))
            {
                ...
                {
                    std::cerr << "encountered the value " << key
                              << "twice in the config file" << std::endl;
                    exitFunction(-1, true);
                }
                else
                    userValue[t] = val;
...
void                    BlockProposalConfig::verify()
{
    ...
    {
        tout << "Error: >esprStopProp< must be in the range (0.01,1]"
             << std::endl;
        exitFunction(-1, true);
    }
    ...
    if (not (0.01 < esprStopProp && esprStopProp <= 1))
    {
        tout << "Error: >etbrStopProb< must be in the range (0.01,1]"
             << std::endl;
        exitFunction(-1, true);
    }
    ...
    {
        tout << "Error: >parsimonyWarp< must be in the range (0.001,10)"
             << std::endl;
        exitFunction(-1, true);
    }
    ...
    {
        tout << "Error: >parsSPRRadius< must be in the range (1,inf)"
             << std::endl;
        exitFunction(-1, true);
    }
}
