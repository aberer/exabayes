...
void                            Path::debug_assertPathExists(
    TreeAln& traln)
{
...
void                    Path::findPath(
    ...
    nodeptr        q)
{
    // cout << "trying to find path between " << p->number << "/" <<
    // p->back->number << " and "  << q->number << "/" << q->back->number  <<
    // endl;
    ...
    if (found)
        append(BranchPlain(p->next->next->number,
