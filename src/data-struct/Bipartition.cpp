...
size_t            Bipartition::numBitsMinusOne = numBits - 1; // not esthetic,
                                                              // but efficient
...
bool                            Bipartition::operator==(
    const Bipartition&rhs) const
{
    // TODO ui ui ui that's inefficient...
    // size cannot be used at all here. And hash woudl be nice, but definitely
    // makes trouble with operator&
...
void                            Bipartition::initializeWithTaxon(
    nat pos,
    nat ranNum)
{
    reserve(pos + 1);
    ...
    size_t num)
{
    ...
    // std::cout  << "have " << initSize << " needing " << elemsNeeded <<
    // (didResize ? " => RESIZE " : "")<< std::endl;
}

void                            Bipartition::set(
    nat pos)
{
...
Bipartition                     Bipartition::getComplement(
    nat maxElem) const
{
    auto result = Bipartition{};
