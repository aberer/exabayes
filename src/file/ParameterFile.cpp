    ...
    : runid(ru)
{
    ...
    ss << OutputFile::getFileBaseName(workdir) << "_parameters.run-"
       << runid << "." << runname;
    fullFileName = ss.str();
}


void                    ParameterFile::initialize(
    const TreeAln&       traln,
    ...
    bool                 isDryRun)
{
    ...
    auto&& fh =  std::ofstream{
        fullFileName, std::fstream::out
    };
