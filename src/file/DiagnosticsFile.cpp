...
std::string                    DiagnosticsFile::createName(
    std::string runname,
    std::string workdir)
{
    std::stringstream ss;
...
void                           DiagnosticsFile::initialize(
    std::string                      workdir,
    std::string                      name,
...
void                    DiagnosticsFile::printDiagnostics(
    uint64_t                         gen,
    double                           asdsf,
    const std::vector<CoupledChains>&runs)
{
        ...
        for (auto&chain: run.getChains())
        {
            ...
            for (auto&ps  :chain.getProposalSets())
            {
                for (auto&p : ps.getProposalView())
                {
                    ...
                    assert(name2proposal.find(oss.str()) ==
                           name2proposal.end());
...
static std::vector<std::string>                    getProposalNames(
    std::string line)
{
...
void                                               DiagnosticsFile::regenerate(
    std::string workdir,
    std::string nowId,
    std::string prevId,
    uint64_t    gen)
