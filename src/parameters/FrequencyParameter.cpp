...
void                                FrequencyParameter::applyParameter(
    TreeAln&               traln,
...
ParameterContent                    FrequencyParameter::extractParameter(
...
void                                FrequencyParameter::printSample(
...
void                                FrequencyParameter::printAllComponentNames(
    std::ostream& fileHandle,
    const TreeAln&traln)  const
{
    auto                     content = extractParameter(traln);
...
void                    FrequencyParameter::verifyContent(
    const TreeAln&         traln,
    const ParameterContent&content) const
{
    ...
    if (not ok)
    {
        tout << "ERROR: we had " << content << " for parameter " << this
             << ". Did you mis-specify a fixed prior?"  << std::endl;
        assert(0);
    }
}


void                    FrequencyParameter::checkSanityPartitionsAndPrior(
    const TreeAln&traln) const
{
