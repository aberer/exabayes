...
void                                RevMatParameter::applyParameter(
    TreeAln&               traln,
...
ParameterContent                    RevMatParameter::extractParameter(
...
ParameterContent                    RevMatParameter::extractParameterRaw(
    const TreeAln& traln) const
{
...
void                                RevMatParameter::applyParameterRaw(
    TreeAln&                traln,
...
void                                RevMatParameter::printSample(
...
void                                RevMatParameter::printAllComponentNames(
    std::ostream& fileHandle,
    const TreeAln&traln) const
{
    auto                     content = extractParameter(traln);
    ...
    default:
    {
        std::cerr << "error in revMatParameter: encountered case >" <<  size
                  << std::endl;
        assert(0);
    }
    }
...
void                    RevMatParameter::verifyContent(
    const TreeAln&         traln,
    const ParameterContent&content) const
{
    ...
    auto  num = RateHelper::numStateToNumInTriangleMatrix(
            partition.getStates());
    ...
    if (not ok)
    {
        tout << "Wrong content " << content << " for parameter "
             << this
             <<
            ". Did you mis-specify a fixed prior or are your input values to extreme?"
             << std::endl;
        assert(0);
    }
}
...
void                    RevMatParameter::checkSanityPartitionsAndPrior(
    const TreeAln&traln) const
{
    ...
    if (initVal.values.size() != RateHelper::numStateToNumInTriangleMatrix(
            numStates) &&
        initVal.protModel.size() == 0)
    {
        tout << "Error while processing parsed priors: you specified prior "
             << _prior.get() << " for parameter ";
