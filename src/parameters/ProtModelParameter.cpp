...
void                                ProtModelParameter::applyParameter(
    TreeAln&               traln,
...
ParameterContent                    ProtModelParameter::extractParameter(
...
void                                ProtModelParameter::printSample(
...
void                                ProtModelParameter::printAllComponentNames(
    std::ostream& fileHandle,
    const TreeAln&traln) const
{
    fileHandle << "aaModel{";
...
void                                ProtModelParameter::verifyContent(
    const TreeAln&         traln,
    ...
    const TreeAln&traln) const
{
    ...
    {
        std::cerr << "Error: in the config file you specified partition "
                  << _partitions.at(0)
                  <<
        " to have an amino acid model. However, previously this partition was declared to have a different data type."
                  << std::endl;
        exitFunction(-1, true);
    }
}
...
bool                                ProtModelParameter::fitsToPartition(
    Partition& p) const
{
