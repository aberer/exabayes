...
std::ostream&                   AbstractParameter::printShort(
    ...
    const TreeAln&traln) const
{
    ...
    {
        std::cerr
            << "Error while processing parsed parameters: you tried to link "
            << _partitions[0] << " and " <<  wrong
            <<
            ". These partitions have a different number of states (probably DNA and PROT). Aborting."
            << std::endl;
        exitFunction(-1, true);
    }
}
...
bool                            AbstractParameter::priorIsFitting(
