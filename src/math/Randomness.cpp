...
double                       Randomness::drawRandBiUnif(
    double x)
{
...
double                       Randomness::drawMultiplier(
    double multiplier)                               //
{
...
double                       Randomness::drawFromSlidingWindow(
    double value,
    double window)
{
...
void                         Randomness::drawPermutation(
    int* perm,
    int  n)
{
    int i;
...
// This function should be called if the alphas for the dirichlet distribution
// are given
std::vector<double>                    Randomness::drawRandDirichlet(
...
double                                 Randomness::drawRandGamma(
    double alpha,
    double beta)
{
    ...
    generator.seed((*this)());
    auto dist = std::gamma_distribution<double>(alpha, 1. / beta); // 1/beta
                                                                   // necessary?
    return dist(generator);
}


nat                                    Randomness::drawGeometric(
    double prop)
{
...
{
    out << "key={" << rhs.key.v[0] << "," << rhs.key.v[1] << "},ctr={"
        << rhs.ctr.v[0] << "," << rhs.ctr.v[1] << "}";
    return out;
}


void                                   Randomness::deserialize(
    std::istream&in)
{
...
void                                   Randomness::rebaseForGeneration(
    uint64_t generation)
{
    // TODO
...
double                                 Randomness::drawRandWeibull(
    double lambda,
    double k)
{
