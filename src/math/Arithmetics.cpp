    ...
    const std::vector<std::vector<double> >data)
{
    ...
    for (nat i = 0; i < means.size(); ++i)
        betweenChainVariance += double(data[i].size()) * pow(means[i]
                                                             - meanOfMeans, 2);
    ...
    estimatedVariance = (1. - 1. / meanNumberOfDraws) * withinChainVariance
        + betweenChainVariance / double(meanNumberOfDraws);
...
double                    Arithmetics::getKahansSum2(
    const std::vector<double>&x)
{
    ...
    for (nat i = 0; i < x.size(); ++i)
    {
        ...
        double c = (std::fabs(s) >= std::fabs(x[i])) ?  (s - t) + x[i]  :
            (x[i] - t) + s;
        s = t;
        t = cs + c;
        double cc  = (std::fabs(cs) >= std::fabs(c)) ?  (cs - t) + c : (c
                                                                        - t)
            + cs;
        cs   = t;
        ccs += cc;
...
double                    Arithmetics::getKahanSum(
    const std::vector<double>&x)
{
    auto sum = 0.;
    auto c = 0.;
