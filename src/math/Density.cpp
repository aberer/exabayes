...
static log_double                    betaFunctionLog(
...
// double dirichlet(std::vector<double> values, const std::vector<double>
// &alphas)
// {
//   assert(0);             // deprecated
//   double density=1;
//   density /= betaFunction(alphas);
//   assert(fabs(std::accumulate(values.begin(), values.end(), 0.) -  1.0 )  <
// 1e-6);
...
log_double                           lnGamma(
    ...
    double beta)
{
    auto result =  (alpha - 1.0) * log(x) + alpha * log(beta) - x * beta
        - logGamma(alpha).getRawLog();
    ...
    double k)
{
    if (x < 0)
        ...
        return log_double::fromLog(log(k / lambda)  + (k - 1) * log(x
                                                                    / lambda)
                                   - pow(x / lambda, k));
}
