...
WeibullProposer::WeibullProposer(
    double nrOpt,
    double nrd1,
    double nrd2,
        ...
        auto   lam = [=](double v) -> double
            {
                return pow(_nrOpt, 2)
                       * pow((v - 1) / v,  -2 / v)
                       * (Density::gammaFunction(1 + (2 / v)) - pow(
                              Density::gammaFunction(1 + (1 / v)), 2))
                       - estVar;
            };
...
BranchLength                    WeibullProposer::proposeBranch(
    BranchPlain        b,
    TreeAln&           traln,
    ...
    Randomness&        rand) const
{
    ...
    return BranchLength(b,
                        InternalBranchLength::fromAbsolute(
                            proposedVal,
                            param->getMeanSubstitutionRate()));
}

log_double                      WeibullProposer::getLogProbability(
    double val) const
{
