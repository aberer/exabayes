...
InsertionResult                    EvalPlain::getResult(
...
InsertionResult                    EvalOptDragged::getResult(
    LikelihoodEvaluator&                  eval,
    TreeAln&                              traln,
    const BranchPlain&                    insertBranch,
    const BranchPlain&                    prunedSubtree,
    ...
    const std::vector<AbstractParameter*>&params) const
{
    ...
    auto blo = BranchLengthOptimizer(traln, draggedBranch, MAX_OPT_ITER,
                                     eval.getParallelSetup().getChainComm(),
                                     params);
    ...
    for (auto&optParam : optParams)
    {
        ...
        // tout << "optimized: " << b.toPlain() << " => " <<  MAX_SCI_PRECISION
        // << b.getInterpretedLength(traln, p) <<  std::endl;
...
#ifdef PRINT_LIKESPR_INFO
    tout << SOME_FIXED_PRECISION << "INSERT " << insertBranch << "\t" << SHOW(
        initLnl) << "\tnow=" << traln.getLikelihood() << "\tdiff="
         << traln.getLikelihood() / initLnl << MAX_SCI_PRECISION  << " with "
         << optParams[0].getOptimizedBranch().getInterpretedLength(traln,
                                                              optParams[0].
                                                                  getParam())
         << " "
         << optParams[0].getOptimizedBranch().getLength() << std::endl;
#endif

    return InsertionResult{
               insertBranch, lnl, optParams
    };
}


InsertionResult                    EvalOptThree::getResult(
    LikelihoodEvaluator&                  eval,
    TreeAln&                              traln,
    const BranchPlain&                    insertBranch,
    const BranchPlain&                    prunedSubtree,
    ...
    const std::vector<AbstractParameter*>&params) const
{
    ...
    auto branches = std::vector<BranchPlain>{
        prunedSubtree, draggedBranch, traln.getThirdBranch(prunedSubtree,
                                                           draggedBranch)
    };
...
// #endif
    ...
    while (not converged &&  ctr < 10)
    {
        ...
        for (auto branch : branches)
        {
            ...
            auto blo = BranchLengthOptimizer(traln, branch, MAX_OPT_ITER,
                                             eval.getParallelSetup().
                                                 getChainComm(), params);
            blo.optimizeBranches(traln);
    ...
    tout << SOME_FIXED_PRECISION << "INSERT " << insertBranch << "\t" << SHOW(
        initLnl) << "\tnow=" << traln.getLikelihood() << "\tdiff="
         << traln.getLikelihood() / initLnl << MAX_SCI_PRECISION  << std::endl;
#endif
