 ...
 */
BranchSetProposer::BranchSetProposer(
    TreeAln&                       traln,
...
void                    BranchSetProposer::findJointOptimum(
    ...
    bool                 computeLikelihood)
{
    ...
    while (not conv)
    {
        ...
        for (auto b : _branches)
        {
            ...
            auto blo = BranchLengthOptimizer(_traln, b, B_MAX_IT, comm,
                                             _params);
            ...
            for (auto optParam : optParams)
            {
                ...
                auto lenNow = optBranch.toMeanSubstitutions(
                        p->getMeanSubstitutionRate());
                auto lenOld = _traln.get().getBranch(b, p).toMeanSubstitutions(
                        p->getMeanSubstitutionRate());
                // tout << ctr << "\t" << b << "\t" << MAX_SCI_PRECISION <<
                // SHOW(lenNow) << SHOW(lenOld) << std::endl;
                ...
                branchesConverged &=   std::fabs(log(lenNow) - log(lenOld)) <
                    THRESH;
            }
        ...
        conv = _branches.size() == 1 ||  (ctr == maxIter) ||
            branchesConverged;
        ++ctr;
    }
...
void                    BranchSetProposer::reorderToConnectedComponent()
{
                            ...
                            [&](const BranchPlain&b)  -> bool
    {
        if (isOuter(b))
        {
            ...
            auto numNeighborOuter = std::count_if(begin(neighbors), end(
                                                      neighbors), [&](const
                                                                      BranchPlain
                                                                      & bb)
            {
