...
void                                              BranchBackup::resetFromBackup(
    TreeAln& traln) const
{
    for (auto& b: _backup)
...
// meh
std::tuple<bool, BranchLength>                    BranchBackup::find(
    const BranchPlain&branch,
    AbstractParameter*param) const
{
    auto foundIter =
        std::find_if(begin(_backup),
                     end(_backup),
                     [&](const std::pair<AbstractParameter*,
                                         BranchLength>&elem)
    {
