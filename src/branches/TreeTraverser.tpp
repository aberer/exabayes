    ...
    , _prunedSubtree{prunedSubtree}
{
    ...
    tout << "created tree traverser for " << SHOW(prunedSubtree) << SHOW(
        rootOfTraversed) << std::endl;
#endif
}
...
void TreeTraverser<T                    >::traverse()
{
    ...
    std::tie(floatingBranch, branchAfterPrune) = _traln.get().pruneSubtree(
            _prunedSubtree, _rootOfTraversed, _params);

    testInsert(branchAfterPrune, floatingBranch, true, 0);

    _traln.get().insertSubtree(_prunedSubtree, branchAfterPrune,
                               floatingBranch, _params);
}
 ...
 */
template<class T>
void TreeTraverser<T                    >::testInsert(
    ...
    int               curDepth)
{
    ...
    if (not isFirst || _doFirst)
    {
        ...
        floatingBranch = BranchLengths(BranchPlain(
                                           _prunedSubtree.getPrimNode(),
                                           insertBranch.getPrimNode()),
                                       floatingBranch.getLengths());

        _traln.get().insertSubtree(_prunedSubtree,  insertBranch,
                                   floatingBranch, _params);

        auto dirtyNodes = std::vector<nat>{
            _prunedSubtree.getPrimNode(),
             insertBranch.getPrimNode()
        };
        ...
        auto insert = _insertEval.getResult(_eval, _traln, insertBranch,
                                            _prunedSubtree, floatingBranch,
                                            _params);
        ...
        std::tie(floatAfterPrune, branchAfterPrune) =
            _traln.get().pruneSubtree(_prunedSubtree, floatingBranch, _params);
