  ...
  : integrationChain{nullptr}
{
  ...
  proposals[0]->addPrimaryParameter( 0 ); 

  auto pSets = std::vector<ProposalSet>{}; 

  auto &&paramList =  ParameterList(std::move(params)); 
  auto bla = new Chain( seed, traln, std::move(paramList), proposals, pSets, std::move(eval), false ) ; 
  integrationChain = std::unique_ptr<Chain> (bla);
}
...
std::vector<double> AdHocIntegrator::integrate( const BranchPlain &branch, const TreeAln &otherTree)
{
  ...
  while(not converged)
	...
	{	  
	  ...
	  auto iLen = elem.getInterpretedLength( paramView[0]);
	  if (i % 10 == 0)
...
double AdHocIntegrator::printOptimizationProcess(const BranchLength& branch, std::string runid, nat nrSteps, Communicator& comm)
{
  ...
  double curVal = 0.1; 
  tmpBranch.setConvertedInternalLength(paramView[0], curVal); 
...
void AdHocIntegrator::createLnlCurve(BranchPlain branch, std::string runid, TreeAln & traln , double minHere, double maxHere, nat numSteps)
{
  ...
  if(  std::fabs(maxHere - minHere) > std::numeric_limits<double>::epsilon() )
    {
      for(double i = minHere; i < maxHere+0.00000001 ; i+= (maxHere-minHere)/ numSteps)
	{
	  auto b = traln.getBranch(branch, param); 
	  b.setConvertedInternalLength( paramView[0], i); 
