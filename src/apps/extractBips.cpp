    ...
    std::ostream&out)
{
    out
        <<
        "extractBips is a utility for extracting bipartitions, branch lengths\n"
        <<
    ...
    out
        <<
        "         -b relBurnin     proportion of trees to discard as burn-in (beginning at\n"
        <<
        "                          the start of the file). Default: 0.25\n\n";
}

static std::tuple<std::string, std::vector<std::string>,
                  double>                    processCommandLine(
    int   argc,
    char**argv)
{
    double burnin = 0.25;
    ...
    if (not (0 <= burnin && burnin < 1.))
    {
        std::cerr
            <<
            "The relative burn-in to be discarded must be in the range [0,1)."
            << std::endl;
        exitFunction(-1, false);
    }
