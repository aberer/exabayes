    ...
    std::ostream&out)
{
    out
        ...
        "\n"
        "USAGE: ./credibleSet -n id -f treeFile[..] [ -c credibleInterval]\n"
        "\n"
        "           -n id         a run id for the output file\n"

        "           -c int        credible set percentile (in (0,100], default:50)\n"
        "           -f file[..]   one or many topology input file(s)\n";
}


static auto                    processCommandLine(
    int   argc,
    char**argv)
    ->std::tuple<std::string, std::vector<std::string>, nat>
{
    ...
    if (id.compare("") == 0)
    {
        std::cerr << "Error: please specify an id for output files via -n"
                  << std::endl;
        exitFunction(-1, false);
    }
    ...
    if (not (0 < credSet && credSet <= 100))
    {
        std::cerr
            <<
            "Error: the percentile (passed via -c) must be between 0 (excluded) and 100 (included)."
            << std::endl;
        exitFunction(-1, false);
    }
    ...
    char**argv)
{
    ...
    {
        std::cerr << "The file " << ss.str()
                  <<
            " already exists (possibly left over from a previous run). Please\n"
