...
// #endif
    ...
    if (constBurninWasSet && relBurninWasSet)
    {
        std::cout
            <<
            "Error: you cannot set the relative burn-in AND a range of trees to\n"
            << "use.\n";
        exit(-1);
    }
    ...
    if (relBurninWasSet && not (0. <= relBurnin && relBurnin < 1.))
    {
        std::cout
            <<
            "Error: please specifify a value (percentage) between (0,1) for the relative burn-in. Your value was "
            << relBurnin << std::endl;
        exit(-1);
    }
    ...
    std::transform(begin(files), files.end(), begin(numTrees), [&](const
                                                                   std::string&
                                                                   file)
    {
        return sdsf.getNumTreeAvailable(file);
    });
    ...
    for (auto i = 0u; i < files.size(); ++i)
    {
        if (constBurninWasSet)
        {
            ...
            {
                std::cout << "you are trying to discard " << constBurnin
                          <<
                    " trees, but the minimum trees available in one of the files is just "
                          << numTrees[i] << std::endl;
                exit(-1);
            }
        ...
        if (not quiet)
            std::cout << "using trees number " << start[i] <<  " to "
                      << numTrees[i] << " for file " << files[i] << std::endl;
    }
    ...
    if (not quiet)
    {
        std::cout << "average deviation of split frequencies: "
                  <<  sdsfResult.first * 100  << "%" << std::endl;
        std::cout  << "maximum deviation of split frequencies: "
                   << sdsfResult.second * 100 << "%" << std::endl;
        std::cout << "ignored splits that occurred in less than "
                  << ignoreFreq * 100
                  << "% of the trees for any of the specified files."
                  << std::endl;
    }
    else
        std::cout << sdsfResult.first * 100  << "\t" << sdsfResult.second
            * 100 << std::endl;
