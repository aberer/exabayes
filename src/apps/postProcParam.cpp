...
public:
    ...
    size_t                                                        size() const
    {
        return std::max(_values.size(), _models.size());
    }
...
static std::unordered_map<std::string, Values>                    readFile(
    std::string file,
    double      burnin)
{
    ...
    for (auto&elem : result)
    {
        ...
        if (isAAMod(headers[ctr]))
        {
            vals._models.erase(begin(vals._models), begin(vals._models) + nat(
                                   burnin * double(vals._models.size())));
        }
        else
            vals._values.erase(begin(vals._values), begin(vals._values) + nat(
                                   burnin * double(vals._values.size())));
    ...
    std::ostream&out)
{
    ...
    out
        <<
        "    -b relBurnin      proportion of samples to discard as burn-in (beginning from start of the file). Default: 0.25\n";
    out << "    -f file[..]       one or many ExaBayes_parameters* files\n\n";
}


static std::tuple<std::string, std::vector<std::string>,
                  double>                    processCmdLine(
    int   argc,
    char**argv)
{
    int  c  = 0;
    ...
    if (files.size()  == 0)
    {
        std::cerr << "Error: please specify one or many files via -f."
                  << std::endl;
        exitFunction(-1, false);
    }
    ...
    if (not (0. <= burnin && burnin < 1.))
    {
        std::cerr
            <<
            "Error: the relative burn-in parameter must be in the range [0,1)."
            << std::endl;
        exitFunction(-1, false);
    }
    ...
    char**argv)
{
    ...
    {
        std::cerr << "Error: output file >" << outputFileName
                  << "< already exists. Please or move." << std::endl;
        exitFunction(-1, false);
    }

    auto&& out = std::ofstream(outputFileName);
    auto   headerToValues = std::vector<std::unordered_map<std::string,
                                                           Values> >{};
    ...
    for (auto header : headers)
    {
        ...
        for (auto headerToSomeValues : headerToValues)
        {
            ...
            if (isAA)
            {
                valuesConcat._models.reserve(valuesConcat.size()
                                             + someVals.size());
                valuesConcat._models.insert(end(valuesConcat._models), begin(
                                                someVals._models), end(
                                                someVals._models));
            }
            else
            {
                valuesConcat._values.reserve(valuesConcat.size()
                                             + someVals.size());
                valuesConcat._values.insert(end(valuesConcat._values), begin(
                                                someVals._values), end(
                                                someVals._values));
            }
        }
        ...
        else        // print values
        {
            ...
            auto sd = std::sqrt(Arithmetics::getVariance(
                                    valuesConcat._values));
            auto perc95  = Arithmetics::getPercentile(.95,
                                                      valuesConcat._values);
            auto perc5   = Arithmetics::getPercentile(.5,
                                                      valuesConcat._values);
            auto perc50  = Arithmetics::getPercentile(.50,
                                                      valuesConcat._values);
            auto perc25  = Arithmetics::getPercentile(.25,
                                                      valuesConcat._values);
            auto perc75  = Arithmetics::getPercentile(.75,
                                                      valuesConcat._values);
            auto ess  = Arithmetics::getEffectiveSamplingSize(
                    valuesConcat._values);
    ...
    std::cout << "successfully printed statistics to file >"
