...
{
    out << "\n"
           "consense computes various flavours of consensus trees from sets"
           "of trees.\n"
           "\n"
           "Usage: ./consense -n id  -f file[..] [-t threshold] [-b burnin]\n"
           "\n"
           "        -n runid         an id for the output file\n"
           "        -t thresh        a threshold for the consenus tree. "
           "Valid values:\n"
           "                         values between 50 (majority rule) and 100"
           " (strict) or MRE\n"
           "                         (the greedily refined MR consensus).  "
           "Default: MRE\n"
           "        -b relBurnin     proportion of trees to discard as burn-in"
           " (from start). Default: 0.25\n"
           "        -f file[..]      one or more exabayes topology files\n\n";
}


static std::tuple<std::string, std::vector<std::string>, double, nat,
    ...
    char**argv)
{
    ...
    {
        std::cerr << "error: correct values for -t in [50,100] or MRE."
                  << std::endl;
        exitFunction(-1, false);
    }
    ...
    if (id.compare("") == 0)
    {
        std::cerr << "Please specify a runid for the output via -n. "
                  << std::endl;
        exitFunction(-1, false);
    }
    ...
    char**argv)
{
    ...
    std::tie(id, files, burnin, threshold, isMre) = processCommandLine(argc,
                                                                       argv);
    ...
    auto&&      outfile2 = std::ofstream{
        ss2.str()
    };
    outfile2 << ct.getConsensusTreeString(true) << std::endl;


    std::cout << "Printed consensus tree in nexus format to " << ss.str()
              << std::endl;
    std::cout << "Printed consensus tree in newick format to " << ss2.str()
              << std::endl;
