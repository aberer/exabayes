    ...
    char**argv)
{
    ...
    if (outputFileName.compare("") != 0 && std::ifstream(outputFileName
                                                         + ".binary"))
    {
        std::cout << "Error: output file "  <<  outputFileName
                  << ".binary already exists." << std::endl;
        exitFunction(-1, false);
    }
    ...
    if (modelFile.compare("") == 0 && singlePartitionModel.compare("") == 0)
    {
        std::cout
            <<
            "Please specify either a model file via -q or a data type for a single\n"
    ...
    {
        std::cout
            << "Error: model file provided, but could not open model file >"
            << modelFile << "<" << std::endl;
        exitFunction(-1, false);
    }
    ...
    {
        std::cout << "Error: could not open alignment file >"
    ...
    std::cout << "wrote binary alignment file to " << outputFileName
              << ".binary" << std::endl;
