...
struct hash<AbstractParameter*>
{
    // public:
    size_t                    operator()(
        const AbstractParameter* rhs) const
    {
        return rhs->getId();
    }
};
...
{
    bool                      operator()(
        const AbstractParameter* a,
        const AbstractParameter* b) const
    {
...
std::ostream&                       Chain::addChainInfo(
    std::ostream&out) const
{
    return out << "[run=" << _runid << ",heat=" << _couplingId << ",gen="
               << getGeneration() << "]";
}
...
void                                Chain::resume()
{
    ...
    if (std::fabs(log_double(_lnPr / prNow).toAbs()) - 1. >= ACCEPTED_LNPR_EPS)
    {
        ...
        std::cerr
            <<
            "While trying to resume chain: previous log prior could not be\n"
            << "reproduced. This is a programming error." << std::endl;
        std::cerr << "Prior was " << _lnPr << "\t now we have " << prNow
                  << "\tdiff=" << std::fabs(_lnPr.getRawLog()
                                  - prNow.getRawLog())
                  << std::endl;
        assert(0);
    }
...
void                                Chain::serializeConditionally(
    std::ostream&out,
...
void                    Chain::deserializeConditionally(
    std::istream& in,
    CommFlag      commFlags)
{
    if (( commFlags& CommFlag::PRINT_STAT ) != CommFlag::NOTHING)
    ...
    if (( commFlags& CommFlag::PROPOSALS ) != CommFlag::NOTHING)
    {
        ...
        for (auto&p : _params)
            p->deserialize(in); // dubios decision: serialized version of a

        // parameter could also be the content...
    }

    if (( commFlags& CommFlag::TREE ) != CommFlag::NOTHING)
    {
        auto param2content = std::unordered_map<AbstractParameter*,
                                                ParameterContent>{};
        ...
        for (auto&param : _params)
        {
            ...
            auto content = param->extractParameter(_traln); // initializes the
                                                            // object
                                                            // correctly. the
                                                            // object must
                                                            // "know" how many
...
BranchPlain                    Chain::peekNextVirtualRoot(
    TreeAln&   traln,
    Randomness rand)
{
...
#endif
        // tout << "no prediction, just returning a somewhat inner branch " <<
        // branch  << std::endl;
    }
    else
    {
...
void                    Chain::stepSingleProposal()
{
    ...
    double acceptance = fmin(log_double(exponentiate(priorRatio * lnlRatio,
                                                     getChainHeat())
                                        * _hastings).toAbs(), 1.);
    ...
    addChainInfo(output);
    output << "\t" << (wasAccepted ? "ACC" : "rej")  << "\t"
           << pfun.getName() << "\t"
           << MORE_FIXED_PRECISION << SHOW(prevLnl) << SHOW(lnlRatio) << SHOW(
        priorRatio) << SHOW(_hastings) << SHOW(acceptance)  << SHOW(testr)
           << std::endl;
#endif
    ...
    else
    {
        ...
        auto myRejected = std::vector<bool>(traln.getNumberOfPartitions(),
                                            false);
 ...
 */
void                    Chain::applyParameterContents(
    std::unordered_map<AbstractParameter*, ParameterContent>param2content)
{
    auto topoParams =  std::unordered_map<AbstractParameter*,
                                          ParameterContent>{};
    auto otherParams = std::unordered_map<AbstractParameter*,
                                          ParameterContent>{};
...
void                    Chain::stepSetProposal()
{
    ...
    auto branches  = proposals.at(_chainRand.drawIntegerOpen(
                                      proposals.size()))->
            prepareForSetExecution(_traln, _chainRand);
    ...
    for (auto&proposal: proposals)
    {
        ...
        affectedPartitions.insert(end(affectedPartitions), begin(partitions),
                                  end(partitions));
    }
    ...
    if (proposals[0]->isUsingOptimizedBranches())
    {
        ...
        _evaluator.evaluateSubtrees(_traln, relevantBranch, affectedPartitions,
                                    false);

        _evaluator.evaluatePartitionsDry(_traln, relevantBranch,
                                         affectedPartitions);

        auto blo = BranchLengthOptimizer(_traln, relevantBranch, 30,
                                         _evaluator.getParallelSetup().
                                             getChainComm(),
                                         affectedParameters);
        ...
        ) // TODO another HACK
    {
        ...
        _evaluator.evaluatePartitionsWithRoot(_traln, nextRoot,
                                              affectedPartitions,
                                              fullTraversalNecessary);
    }
    else
    {
        pSet.getProposalView()[0]->prepareForSetEvaluation(_traln, _evaluator);
        _evaluator.evaluatePartitionsWithRoot(_traln, branches.first,
                                              affectedPartitions,
                                              fullTraversalNecessary);
    }
    ...
    auto partitionsToReset = std::vector<bool>(_traln.getNumberOfPartitions(),
                                               false);
    nat  accCtr = 0;
    nat  total  = 0;
    ...
    for (auto&proposal : pSet.getProposalView())
    {
        ...
        auto accRatio = log_double(exponentiate(p2LnPriorRatio[proposal]
                                                * (newLnl
                                                   / p2OldLnl[proposal]),
                                                getChainHeat())
                                   * p2Hastings[proposal]).toAbs();
...
const std::vector<AbstractProposal*>                    Chain::getProposalView()
const
{
    ...
    const Chain&  rhs)
{
    ...
    // out  << "\tLnl: " << rhs.tralnPtr->getTr()->likelihood << "\tLnPr: " <<
    // rhs.prior.getLnPrior();
    ...
    ParameterFile&pFile) const
{
    ...
    for (auto&param : _params)
    {
        ...
        else if (param->getCategory() == Category::TOPOLOGY &&
                 param->getPrior()->needsIntegration())
...
void                                                    Chain::serialize(
    std::ostream&out) const
{
    serializeConditionally(out, CommFlag::PRINT_STAT | CommFlag::PROPOSALS
                           | CommFlag::TREE | CommFlag::RAND);
}


void                                                    Chain::deserialize(
    std::istream&in)
{
    deserializeConditionally(in, CommFlag::PRINT_STAT | CommFlag::PROPOSALS
                             | CommFlag::TREE | CommFlag::RAND);
}
