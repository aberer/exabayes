...
bool                                 SampleMaster::initializeTree(
    TreeAln&                         traln,
    std::string                      startingTree,
    Randomness&                      treeRandomness,
    const vector<AbstractParameter*>&params)
{
...
std::vector<bool>                    SampleMaster::initTrees(
    vector<TreeAln>&                 trees,
    randCtr_t                        seed,
    vector<std::string>              startingTreeStrings,
    const vector<AbstractParameter*>&params)
{
    ...
    {
        auto stTr = treesConsumed < startingTreeStrings.size() ?
            startingTreeStrings.at(treesConsumed) : std::string{
            ""
        };
        ++treesConsumed;
...
void                    SampleMaster::printAlignmentInfo(
    const TreeAln&traln)
{
    tout << endl
         <<
...
void                    SampleMaster::informPrint()
{
    ...
    {
        tout << "Will execute " << _plPtr->getRunsParallel()
             << " runs in parallel." << endl;

        if (nat(_runParams.getNumRunConv()) < _plPtr->getRunsParallel())
        {
            tout
                << "Error: in the configuration file you specified to run "
                <<  _runParams.getNumRunConv() << " independent\n"
                << "runs. Your command line indicates, that you want to run "
                << _plPtr->getRunsParallel() << " of them\n"
                << "in parallel. To shield you from surprises, "
                << PROGRAM_NAME << " will conservatively abort." << endl;
            exitFunction(-1, true);
        }
    }
    ...
    {
        tout << "Will execute " << _plPtr->getChainsParallel()
             << " chains in parallel." << endl;

        if (nat(_runParams.getNumCoupledChains()) <
            _plPtr->getChainsParallel())
        {
            tout
                << "Error: in the configuration file you specified to run "
                <<  _runParams.getNumCoupledChains() << " coupled\n"
                <<
                "chains per run. Your command line indicates, that you want to run "
                << _plPtr->getChainsParallel() << " of them\n"
                << "in parallel. To shield you from surprises, "
                << PROGRAM_NAME << " will conservatively abort." << endl;
            exitFunction(-1, true);
        }
...
void                    SampleMaster::initializeFromCheckpoint()
{
    ...
    if (not chkpnt)
    {
        tout  << "Warning! Could not open / find file " << checkPointFile
              << endl;
        tout
            <<
            "Although extremely unlikely, the previous run may have been killed\n"
            <<
            "during the writing process of the checkpoint. Will try to recover from backup..."
            << endl;

        ss.str("");
        ss << PROGRAM_NAME << "_prevCheckpointBackup."
           << _cl.getCheckpointId();
        ...
        if (not chkpnt)
        {
            tout
                <<
                "Could not recover checkpoint file backup either. Probably there is no backup, since not enough generations have completed. Giving up. Please start a new run."
                << endl;
            ...
            tout
                <<
                "Success! You lost one checkpoint, but we can continue from the backup. "
                << endl;
    }
    ...
    if (_plPtr->isGlobalMaster())
    {
        for (auto&thisRun : _runs)
            thisRun.regenerateOutputFiles(_cl.getWorkdir(),
                                          _cl.getCheckpointId());

        auto curGen = _runs[0].getChains()[0].getGeneration();
        _diagFile.regenerate(_cl.getWorkdir(), _cl.getRunid(),
                             _cl.getCheckpointId(),
                             curGen);
    }
}
...
LikelihoodEvaluator                    SampleMaster::createEvaluatorPrototype(
...
void                    SampleMaster::initializeWithParamInitValues(
    TreeAln&            traln,
    ...
    bool                hasBl) const
{
    ...
    if (hasBl)
    {
        ...
        {
            auto len = traln.getBranch(b, blParam).toMeanSubstitutions(
                    blParam->getMeanSubstitutionRate());
            // tout << b << "\t" << len << endl;
    ...
    for (auto&param : params)
    {
        ...
        auto datatype =  traln.getPartition(
                param->getPartitions()[0]).getDataType();
        ...
        if (not (cat == Category::TOPOLOGY || cat == Category::BRANCH_LENGTHS))
        {
            ...
            // :NOTICE: rev mat parameters for aa-partitions must be prepared
            // here!
    ...
    for (auto&param : params)
    {
        ...
        if (cat == Category::BRANCH_LENGTHS)
        {
            ...
            for (auto belem : branches)
            {
                ...
                auto b = BranchLength(std::get<0>(belem),
                                      InternalBranchLength::fromAbsolute(
                                          absLen,
                                          param
                ...
                if (not BoundsChecker::checkBranch(b))
                {
                    ...
                    auto newLen = b.toMeanSubstitutions(
                            param->getMeanSubstitutionRate());
                    tout << "Warning: had to modify branch length "
                         << absLen << " to " << newLen
                         <<
                        " because it violated the maximum range of branch lengths allowed."
                         << endl;
                }
...
static double                    traverseDepthFromRoot(
    TreeAln&               traln,
    ...
    std::vector<NodeAge*>& nodeAges)
{
    ...
    if (!traln.isTipNode(branch.getPrimNode()))
    {
        ...
        nodeAges[plainDescendants.first.getSecNode() - 1]->setHeight(
            currentHeight + 1.0);
        nodeAges[plainDescendants.second.getSecNode() - 1]->setHeight(
            currentHeight + 1.0);

        return std::max(
            traverseDepthFromRoot(traln, plainDescendants.first.getInverted(),
                                  nodeAges),
            traverseDepthFromRoot(traln, plainDescendants.second.getInverted(),
                                  nodeAges)
            );
    }
    else
...
void                             SampleMaster::makeTreeUltrametric(
    TreeAln&                        traln,
    ...
    std::vector<AbstractParameter*>&divRates) const
{
    ...
    double           maxHeight = std::max(
            traverseDepthFromRoot(traln, traln.getRootBranch(),
                                  nodeAges),
            traverseDepthFromRoot(traln, traln.getRootBranch().getInverted(),
                                  nodeAges));

    /* correct the node heights from the tips to the root and initialize
     * branches */
    for (auto b : nodeAges)
    {
        b->setHeight(
    ...
    auto divtime = static_cast<DivergenceTimes*>(divTimes[divTimes.size()
                                                          - 1]);
    ...
    for (nat i = 0; i < traln.getNumberOfNodes(); i++)
    {
        ratesContent.branchLengths.push_back(BranchLength(*nodeAges[i],
    ...
    ParameterList&                                  params) const
{
    ...
    if (_runParams.isComponentWiseMH() && proposalSets.size() > 0)
    {
        tout
            <<
            "In addition to that, the following sets below will be executed \n"
            <<
...
void                    SampleMaster::printParameters(
    const TreeAln&      traln,
    const ParameterList&params) const
{
    ...
    for (auto&v : params)
    {
        ...
        else if (dynamic_cast<RevMatParameter*>(v)  ||
                 dynamic_cast<FrequencyParameter*>(v))
        {
...
std::string                    SampleMaster::getOrCreateBinaryFile() const
{
    ...
    if (not _cl.alnFileIsBinary())
    {
        ...
        auto modelInfo = haveModelFile ? _cl.getModelFile() :
            _cl.getSingleModel();

        binaryAlnFile = std::string(_cl.getWorkdir()
                                    +  (_cl.getWorkdir().compare("") == 0 ?
                                        "" : "/")
                                    +   "ExaBayes_binaryAlignment" + "."
                                    + _cl.getRunid());
        ...
        if (_plPtr->isGlobalMaster())
        {
            ...
            {
                tout << "removing previous binary alignment representation "
                     << binaryAlnFile << endl;
        ...
        {
            std::cout << "Error: tried to create intermediate file "
                      << binaryAlnFile << ", but did not succeed!"  << endl;
            exitFunction(-1, false);
        }
    }
    else
...
void                    SampleMaster::catchRunErrors() const
{
    ...
    {
        tout << "\n\tERROR: you want to run  " << _cl.getNumRunParallel()
             << " independent runs in parallel, while there are only "
             << _runParams.getNumRunConv() << " to be run in total.\n\n"
             << endl;
        exitFunction(-1, true);
    }
    ...
    {
        tout << "\n\tERROR: you want to run " << _cl.getNumChainsParallel()
             << " coupled chains in parallel, while there are only "
             << _runParams.getNumCoupledChains() << " to be run in total.\n\n"
             << endl;
        exitFunction(-1, true);
    }
    ...
    {
        tout << "\n\tERROR: you specified to run "
             << _runParams.getNumRunConv() << " independent runs and "
             << _cl.getNumRunParallel()
             <<
            " of them in parallel. Currently, the total number of runs must be a multiple of the number of parallel runs."
             << endl;
        exitFunction(-1, true);
    }
    ...
    {
        tout << "\n\tERROR: you specified to run "
             << _runParams.getNumCoupledChains()
             << " coupled chains for each independent run and "
             << _cl.getNumChainsParallel()
             <<
            " of them in parallel. Currently, the total number of coupled chains must be a multiple of the number of parallel coupled chains."
             << endl;
        exitFunction(-1, true);
    }
}
    ...
    Randomness rand)
{
    ...
    {
        std::cerr << "You specified >" << _cl.getRunid()
                  << "< as runid and intended\n"
                  << "to restart from a previous run with id >"
                  << _cl.getCheckpointId() << "<."
    ...
    {
        tout << "You provided " << startingTrees.size() << " starting trees"
             << endl;

        if (topoIsFixed)
            tout
                <<
                "Since the topology is fixed, only the first one will be used."
                << endl;
    }
    ...
    if (topoIsFixed)
    {
        ...
        hadBl = initializeTree(initTree,
                               startingTrees.size() > 0  ? startingTrees.at(
                                   0) : std::string(""),                            //
                                                                                    //
                                                                                    // meh
    ...
    for (nat i = 0; i < _runParams.getNumRunConv(); ++i)
    {
        ...
        auto hadBls = vector<bool>(trees.size(), topoIsFixed  && hadBl ? true :
                                   false);
        ...
        {
            auto&t = trees.at(j);
            chains.emplace_back(runRand.generateSeed(), t, params, proposals,
                                proposalSets, evalUptr, _cl.isDryRun());
            auto&chain = chains[j];
            chain.setRunId(i);
        ...
        _runs.emplace_back(runRand, i, _cl.getWorkdir(), _cl.getRunid(),
                           _runParams.getNumCoupledChains(), chains);
        auto&run = _runs.back();
    ...
    _plPtr->getChainComm().initWithMaxChains(_runParams.getNumCoupledChains(),
                                             _plPtr->getChainComm().size());

    if (_cl.getCheckpointId().compare("") != 0)
        initializeFromCheckpoint();

    if (_plPtr->isGlobalMaster() && not _diagFile.isInitialized() &&
        not _cl.isDryRun())
    ...
    {
        auto res = _plPtr->printLoadBalance(initTree,
                                            _runParams.getNumRunConv(),
                                            _runParams.getNumCoupledChains());
        tout << res;
    }
...
void                    SampleMaster::printInitializedFiles() const
{
    ...
    stream << initString  << " diagnostics file "
           << _diagFile.getFileName()  << endl;
...
{
    auto& res = _runs.at(0).getChains().at(
            0).getEvaluator().getArrayReservoir();
    ...
    nat&end)
{
    if (_runParams.getNumRunConv() == 1)
        return std::pair<double, double>(std::numeric_limits<double>::min(),
    ...
    for (nat i = 0; i < _runParams.getNumRunConv(); ++i)
    {
        ...
        ss << OutputFile::getFileBaseName(_cl.getWorkdir())
           << "_topologies.run-" << i  << "." << _cl.getRunid();

        // if we do not find the file, let's assume we have multiple branch
        // lengths
    ...
    {
        int treesToDiscard =  _runParams.getBurninGen()
            / _runParams.getSamplingFreq();
    ...
    asdsf.extractBips(std::vector<nat>(_runParams.getNumRunConv(), start),
                      std::vector<nat>(_runParams.getNumRunConv(), end));
...
void                    SampleMaster::printDuringRun(
    uint64_t gen)
{
    _plPtr->synchronizeChainsAtMaster(_runs, CommFlag::PRINT_STAT);
    ...
    for (auto&run : _runs)
    {
        ...
        std::sort(begin(sortedLnls), end(sortedLnls), [](const std::pair<nat,
                                                                         log_double>
                                                         &elem1, const
                                                         std::pair<nat,
                                                                   log_double>&
...
void                    SampleMaster::simulate()
{
    if (not _cl.isQuiet())
    {
        ...
        tout << "using the " << impl << " implementation"
             <<    (_cl.isSaveMemorySEV() ?  " with SEVs" : "")
             << " for likelihood computations." << endl;

        if (_runParams.getNumRunConv() > 1 &&  _runParams.isUseStopCriterion())
            tout << PROGRAM_NAME
                 << " will run until topological convergence is achieved\n"
                 << "(" << (_runParams.isUseAsdsfMax() ? "MSDSF < " :
                       "ASDSF < ")
                 << _runParams.getAsdsfConvergence() * 100 <<  "%, at least "
                 << _runParams.getNumGen() << " generations).\n";
        else
            tout  << PROGRAM_NAME << " will run for "
                  << _runParams.getNumGen() << " generations.\n";

        tout << PROGRAM_NAME
             << " will print log-likelihoods of all chains, grouped by\n"
             <<
    ...
    auto lastPrint  = (curGen / _runParams.getPrintFreq())
        * _runParams.getPrintFreq();
    auto lastDiag   =  (curGen / _runParams.getDiagFreq())
        * _runParams.getDiagFreq();
    auto lastChkpnt = (curGen / _runParams.getChkpntFreq())
        * _runParams.getChkpntFreq();
    ...
    while (curGen < nat(_runParams.getNumGen()) || not hasConverged)
    {
        ...
        auto stopPoints = std::vector<uint64_t>{
            nextChkpnt, nextPrint, nextDiag
        };
        ...
        auto nextStop  = *(std::min_element(stopPoints.begin(),
                                            stopPoints.end()));
        ...
        hasConverged = not _runParams.isUseStopCriterion()  ||
            ((_runs.size() == 1) && (curGen >= _runParams.getNumGen()));
        ...
        if ((curGen % _runParams.getDiagFreq()) == 0)
        {
            ...
            // if(not hasConverged)
            {
                if (_plPtr->isGlobalMaster())
                {
                    ...
                    tout << endl
                         <<
                        "standard deviation of split frequencies for trees "
                         << start << "-" << end  << " (avg/max):\t"
                         << PERC_PRECISION << asdsf.first * 100 << "%\t"
                         << asdsf.second * 100 << "%"   << endl << endl;
                }
            }
            else
                hasConverged = true;
...
void                    SampleMaster::finalizeRuns()
{
    for (auto&run : _runs)
    {
        for (auto&chain : run.getChains())
        {
            if (std::fabs(chain.getChainHeat() - 1.) <
                std::numeric_limits<double>::epsilon())
                tout << SOME_FIXED_PRECISION << "best state for run "
                     << run.getRunid() << " was: "  << chain.getBestState()
                     << endl;
        }
    }
    ...
    tout << endl << "Converged/stopped after "
         <<  _runs[0].getChains()[0].getGeneration() << " generations" << endl;
    tout << endl << "Total walltime elapsed:\t"
         <<  SOME_FIXED_PRECISION << wallTimeElapsed << " seconds "
         << "\tor " <<  std::setfill('0') << std::setw(2)
         <<  int(altFormat[0]) << ":" << std::setfill('0') << std::setw(2)
         << int(altFormat[1]) << ":" << altFormat[2] << " (hh:mm:ss)." << endl;
...
void                    SampleMaster::writeCheckpointMaster()
{
    ...
    _plPtr->synchronizeChainsAtMaster(_runs, CommFlag::PRINT_STAT
                                      | CommFlag::PROPOSALS | CommFlag::TREE
                                      | CommFlag::SWAP);
    ...
    if (_plPtr->isGlobalMaster())
    {
        ...
        if (std::ifstream(curName))
        {
            ss.str("");
            ss << OutputFile::getFileBaseName(_cl.getWorkdir())
               << "_prevCheckpointBackup." << _cl.getRunid();
