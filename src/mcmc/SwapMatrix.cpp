...
const SuccessCounter&                   SwapMatrix::getCounter(
    nat a,
    nat b) const
{
    ...
    nat b) const
{
    ...
    if (not (result < matrix.size()))
    {
        tout << "for " << a << "," << b << " the index was "
             << result << ", but matrix has only " << matrix.size()
             << std::endl;
    ...
    const SwapMatrix& rhs)
{
    for (nat i = 0; i < rhs.numEntries - 1; ++i)
    {
        ...
        for (nat j = i + 1; j < rhs.numEntries; ++j)
        {
            ...
            // out << (isFirst ? "" : ",") << std::setprecision(1) << 100 *
...
SwapMatrix                              SwapMatrix::operator+(
