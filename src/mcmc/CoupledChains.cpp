    ...
    ->PendingSwap
{
    ...
    auto   isLocal =   pl.swapIsLocal(theSwap.getOne(), theSwap.getOther(),
                                      _runid);
    ...
    ->bool
{
    ...
    bool   mineHasSmallerId = a.getCouplingId() < b.getCouplingId();
    bool   bothAreMine = pl.isMyChain(_runid, cAIndex) && pl.isMyChain(_runid,
                                                                       cBIndex);

    assert(a.getGeneration() == b.getGeneration());
    assert(b.getChainHeat() <= 1. && a.getChainHeat() <= 1.);

    auto   aB = exponentiate((a.getLikelihood() * a.getLnPr()),
                             b.getChainHeat()),
           bA = exponentiate((b.getLikelihood() * b.getLnPr()),
                             a.getChainHeat()),
           aA = exponentiate((a.getLikelihood() * a.getLnPr()),
                             a.getChainHeat()),
           bB = exponentiate((b.getLikelihood() * b.getLnPr()),
                             b.getChainHeat());
    ...
    ->void
{
    ...
    for (nat i = 0; i < _chains.size(); ++i)
    {
        if (pl.isMyChain(_runid, i))
        {
            ...
            {
                tout << "problem at START was="
                     << _chains[i].getGeneration() << ", should be="
                     << startGen
                     << std::endl;
                assert(0);
            }
    ...
    while (not allSwaps.empty() ||  not pendingSwaps.empty())
    {
        ...
        // check, if a request completed
        {
            ...
            while (psIter != end(pendingSwaps))
            {
                if (psIter->allHaveReceived(pl))
                {
                    ...
                    // MEH
                    auto&&iss = std::istringstream(std::string(begin(
                                                                   serializedRemote),
                                                               end(
                                                                   serializedRemote)));
                    ...
                    swapsToBeDeleted.insert(end(swapsToBeDeleted), std::move(
                                                elem));
        ...
        while (swapIter != end(allSwaps) && not allMyChainsAreBlocked(
                   isBlocked, pl))
        {
            auto swap = *swapIter;
            ...
            else if (oneIsMine)
            {
                ...
                while (not isBlocked[myId] &&  _chains[myId].getGeneration() <
                       gen)
                    doStep(myId, pl);

                if (not isBlocked[myId] && gen ==
                    _chains[myId].getGeneration())
                {
                    pendingSwaps.insert(end(pendingSwaps), std::move(
                                            prepareSwap(pl, swap)));
#ifdef VERBOSE
            ...
            {
                while (not isBlocked[one] && _chains[one].getGeneration() <
                       gen)
                    doStep(one, pl);

                while (not isBlocked[other] && _chains[other].getGeneration() <
                       gen)
                    doStep(other, pl);

                if (not isBlocked[one] && not isBlocked[other]
                    && gen == _chains[one].getGeneration() && gen ==
                    _chains[other].getGeneration())
                {
                    doSwap(pl, swap);
                    swapIter = allSwaps.erase(swapIter);
#ifdef VERBOSE
    ...
    for (nat i = 0; i < _chains.size(); ++i)
    {
        ...
        if (pl.isMyChain(_runid, i))
        {
            ...
            {
                tout << "problem at END was=" << _chains[i].getGeneration()
                     << ", should be=" << startGen << std::endl;
                assert(0);
            }
