...
log_double                    PriorBelief::scoreEverything(
    const TreeAln&traln,
    ...
    ParameterList& parameters)
{
    ...
    {
        tout << MAX_SCI_PRECISION << "ln prior was " << _lnPrior
             << " while it should be " << verified << std::endl;
        tout << "difference: " << fabs(verified.toAbs() - _lnPrior.toAbs())
             << std::endl;
        assert(0);
    }
