...
DiscreteModelPrior::DiscreteModelPrior(
...
log_double                          DiscreteModelPrior::
    accountForMeanSubstChange(
    TreeAln&                 traln,
...
// TODO bool uniform
ParameterContent                    DiscreteModelPrior::drawFromPrior(
...
log_double                          DiscreteModelPrior::getLogProb(
    ...
    std::ostream&out) const
{
    ...
    else
    {
        ...
        for (auto&modelPair : _modelProbs)
        {
            ...
            out << ProtModelFun::getName(std::get<0>(modelPair)) << "="
                << SOME_FIXED_PRECISION <<  std::get<1>(modelPair);
        }
