...
log_double                          UniformPrior::getLogProb(
...
log_double                          UniformPrior::accountForMeanSubstChange(
    TreeAln&                 traln,
    ...
    double                   newFc) const
{
    ...
    for (auto&b : traln.extractBranches(param))
    {
        ...
        if (less || greater)
        {
            // tout << b.getLength() << " is not okay (" <<
            // b.getInterpretedLength(traln,param) << ")" << std::endl;
...
double                              UniformPrior::getFirstDerivative(
