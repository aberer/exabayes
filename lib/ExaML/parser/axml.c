...
int main (int argc, char *argv[])
{
...

  {
    ...
    myBinFwrite(&(tr->mxtips),                 sizeof(int), 1); /*  1  */
    myBinFwrite(&(tr->originalCrunchedLength), sizeof(size_t), 1); /* 2 */
    myBinFwrite(&(tr->NumberOfModels),         sizeof(int), 1); /* 3 */
    myBinFwrite(&(adef->gapyness),             sizeof(double), 1); /* 4 */
    myBinFwrite(tr->cdta->aliaswgt,               sizeof(int), tr->originalCrunchedLength); /* 5 */
	
    for(i = 1; i <= (size_t)tr->mxtips; i++)
      {
	int len = strlen(tr->nameList[i]) + 1;
	myBinFwrite(&len, sizeof(int), 1); /* 6 */
	myBinFwrite(tr->nameList[i], sizeof(char), len); /* 7 */
      }  
	...
	*/
    ...
    for(model = 0; model < (size_t) tr->NumberOfModels; ++model)
      {
    	myBinFwrite( &(tr->partitionData[model].parsimonyLength), sizeof(size_t),1);
    	/* printf("writing parsimony information of length %d\n", tr->partitionData[model].parsimonyLength); */
